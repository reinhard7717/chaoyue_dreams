        ...
        try:
            ...
            if is_incremental:
                try:
                    ...
                    print(f"[{stock_code}] 未找到任何历史指标，自动切换到全量刷新模式。")
            ...
            print(f"[{stock_code}] 数据源诊断: 筹码({cyq_days}天), 行情({daily_days}天), 资金流[CY]({fund_cy_days}天), [THS]({fund_ths_days}天), [DC]({fund_dc_days}天)")
            ...
            if not all_metrics_list:
                print(f"[{stock_code}] 没有需要计算的新指标。任务正常结束。")
            ...
            with transaction.atomic():
                if not is_incremental:
                    print(f"[{stock_code}] 全量模式：删除所有旧数据...")
