...
# --- 新的批量处理工作任务 ---
@celery_app.task(bind=True, name='tasks.ig507.stock_realtime.save_realtime_data_batch')
def save_realtime_data_batch(self, stock_codes: List[str]):
    ...
    try:
        asyncio.run(stock_realtime_dao.fetch_and_save_realtime_data_by_stock_codes(stock_codes))
    except Exception as e:
...
# --- 修改后的调度器任务 ---
@celery_app.task(bind=True, name='tasks.ig507.stock_realtime.save_stocks_realtime_data_task')
def save_stocks_realtime_data_task(self, batch_size: int = 50): # sina数据最多每次50个
    """
    调度器任务：
    1. 获取自选股和非自选股代码。
    2. 将代码分成批次。
    ...
    """
    logger.info(f"任务启动: save_stocks_realtime_data_task (调度器模式) - 获取股票列表并分派批量任务 (批次大小: {batch_size})")
    try:
        # 在同步任务中运行异步代码获取列表
...
@celery_app.task(bind=True, name='tasks.ig507.stock_realtime.save_all_realtime_data_task')
def save_all_realtime_data_task(self):
    """
    获取所有股票的最新实时数据
    """
    stock_realtime_dao = StockRealtimeDAO()
    asyncio.run(stock_realtime_dao.fetch_and_save_all_realtime_data())
