...
async def get_realtime_data_task(self, batch_size: int = 50): # 增加批次大小参数
    """
    调度器任务：
    1. 获取自选股和非自选股代码。
    2. 将代码分成批次。
    ...
    try:
        result = await realtime_dao.fetch_and_save_all_realtime_data()  # 调用异步方法
        return result  # 返回结果
    except Exception as e:
        # Celery 会捕获异常，您可以在这里添加自定义处理
        logger.error(f"执行 get_realtime_data_task (调度器模式) 时出错: {e}", exc_info=True)
        raise self.retry(exc=e, countdown=60)  # 可选：重试机制，60秒后重试
