...
# --- 修改后的调度器任务 ---
@celery_app.task(bind=True, name='tasks.stock_realtime.save_stocks_realtime_data_task')
def save_stocks_realtime_data_task(self, batch_size: int = 200): # 增加批次大小参数
    """
    调度器任务：
    1. 获取自选股和非自选股代码。
    2. 将代码分成批次。
...
@celery_app.task(bind=True, name='tasks.stock_realtime.save_all_realtime_data_task')
