...
@celery_app.task(queue='SaveData_TimeTrade')
def save_stocks_minute_data_yesterday_batch(stock_codes: list):
    """
...
def save_cyq_chips_today_batch():
    """
    从Tushare批量获取实时分钟级交易数据并保存到数据库（异步并发处理）
    Args:
        stock_codes: 股票代码列表
...
# ============== 每日筹码分布任务（昨日） ==============
@celery_app.task(queue='SaveHistoryData_TimeTrade', rate_limit='180/m')
def save_cyq_chips_yesterday_batch():
    """
    从Tushare批量获取实时分钟级交易数据并保存到数据库（异步并发处理）
    Args:
        stock_codes: 股票代码列表
...
@celery_app.task(queue='SaveHistoryData_TimeTrade', rate_limit='180/m')
def save_cyq_perf_yesterday_batch():
    """
    从Tushare批量获取实时分钟级交易数据并保存到数据库（异步并发处理）
    Args:
        stock_codes: 股票代码列表
...
#  ================ （本周）分钟数据任务 ================
@celery_app.task(queue='SaveHistoryData_TimeTrade')
def save_minute_data_this_week_batch(stock_codes: str):
    """
    分钟数据任务（本周）
...
#  ================ （本周）日线数据任务 ================
@celery_app.task(queue='SaveHistoryData_TimeTrade')
def save_day_data_this_week_batch():
    """
    从Tushare批量获取实时分钟级交易数据并保存到数据库（异步并发处理）
    Args:
        stock_codes: 股票代码列表
...
# ============== （本周）每日筹码分布任务 ==============
@celery_app.task(queue='SaveHistoryData_TimeTrade', rate_limit='180/m')
def save_cyq_chips_this_week_batch(ts_code: str, start_date: datetime.date, end_date: datetime.date):
    """
...
@celery_app.task(queue='SaveHistoryData_TimeTrade', rate_limit='180/m')
def save_cyq_perf_this_week_batch(start_date: datetime.date, end_date: datetime.date):
    """
...
def save_minute_data_history_batch(stock_code: str, time_level: str):
    """
    从Tushare批量获取实时分钟级交易数据并保存到数据库（异步并发处理）
    Args:
        stock_code: 股票代码
...
#  ================ 日线数据任务（历史） ================
@celery_app.task(queue='SaveHistoryData_TimeTrade', rate_limit='180/m')
def save_day_data_history_batch(stock_codes: List[str]):
    """
...
#  ================ 每日基本信息数据任务（历史） ================
@celery_app.task(queue='SaveHistoryData_TimeTrade', rate_limit='180/m')
def save_daily_basic_data_history_batch(stock_codes: List[str]):
    """
...
#  ================ 历史(周线)数据任务 ================
@celery_app.task(queue='SaveHistoryData_TimeTrade', rate_limit='480/m')
def save_week_data_history_batch(stock_codes: List[str]):
    """
...
#  ================ 历史(月线)数据任务 ================
@celery_app.task(queue='SaveHistoryData_TimeTrade', rate_limit='180/m')
def save_month_data_history_batch(stock_codes: List[str]):
    """
...
# ============== （历史）每日筹码分布任务 ==============
@celery_app.task(queue='SaveHistoryData_TimeTrade')
def save_cyq_chips_history_batch(ts_code: str, start_date: datetime.date = None, end_date: datetime.date = None):
    """
    从Tushare批量获取实时分钟级交易数据并保存到数据库（异步并发处理）
    Args:
        ts_code: 股票代码
...
def save_cyq_perf_for_stocky_batch(ts_code: str, start_date: datetime.date = None, end_date: datetime.date = None):
    """
