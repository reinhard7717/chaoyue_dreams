...
def save_stocks_daily_basic_data_today_task(self):
    ...
    cache_manager_instance = CacheManager()
    service = IndicatorService(cache_manager_instance)
    try:
        today_date = timezone.now().date()
...
def save_cyq_chips_this_week_batch(self, ts_code: str, start_date: datetime.date, end_date: datetime.date):
    ...
    try:
        ...
        logger.error(f"save_cyq_chips_this_week_batch.执行任务时发生顶层错误 (ts_code: {ts_code}): {e}", exc_info=True)
...
def save_cyq_perf_this_week_batch(self, start_date: datetime.date, end_date: datetime.date):
    ...
    try:
        ...
        logger.error(f"save_cyq_perf_this_week_batch.执行任务时发生顶层错误: {e}", exc_info=True)
...
def save_cyq_data_this_week_task(self):
    ...
    # 2. 在同步代码中，安全地执行异步的 main 函数
    try:
        async_to_sync(get_stocks_main)()
    except Exception as e:
        logger.error(f"在 save_cyq_data_this_week_task 中获取股票列表时出错: {e}", exc_info=True)
...
#  ================ 每日基本信息数据任务（历史） ================
@celery_app.task(bind=True, name='tasks.tushare.stock_time_trade_tasks.save_daily_basic_data_history_batch', queue='SaveHistoryData_TimeTrade', rate_limit='180/m')
...
# ============== （历史）每日筹码分布任务 ==============
@celery_app.task(bind=True, name='tasks.tushare.stock_time_trade_tasks.save_cyq_chips_history_batch', queue='SaveHistoryData_TimeTrade')
def save_cyq_chips_history_batch(self, ts_code: str, start_date: datetime.date = None, end_date: datetime.date = None):
    ...
    try:
        ...
        logger.error(f"save_cyq_chips_history_batch.执行批量保存任务时发生意外错误 (ts_code: {ts_code}): {e}", exc_info=True)
