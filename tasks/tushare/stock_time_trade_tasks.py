...
def save_stocks_minute_data_today_batch(self, stock_codes, trade_time_str=None):
    ...
    try:
        ...
        async_to_sync(stock_time_trade_dao.save_minute_time_trade_history_by_stock_codes)(
...
@celery_app.task(bind=True, name='tasks.tushare.stock_time_trade_tasks.save_stocks_minute_data_yesterday_batch', queue='SaveHistoryData_TimeTrade')
...
def save_stocks_daily_basic_data_today_task(self):
    ...
    try:
        ...
        result = async_to_sync(stock_time_trade_dao.save_stock_daily_basic_history_by_trade_date)(trade_date=today_date)
...
def save_stocks_daily_basic_data_yesterday_task(self):
    ...
    try:
        ...
        result = async_to_sync(stock_time_trade_dao.save_stock_daily_basic_history_by_trade_date)(trade_date=yesterday)
...
def save_day_data_today_task(self):
    ...
    try:
        ...
        result = async_to_sync(stock_time_trade_dao.save_daily_time_trade_history_by_trade_dates)(trade_date=today_date)
...
def save_day_data_yesterday_task(self):
    ...
    try:
        ...
        result = async_to_sync(stock_time_trade_dao.save_daily_time_trade_history_by_trade_dates)(trade_date=yesterday)
...
def save_week_data_yesterday_task(self):
    """
    从Tushare批量获取实时分钟级交易数据并保存到数据库（异步并发处理）
    Args:
        stock_codes: 股票代码列表
    ...
    try:
        ...
        yesterday = today_date - datetime.timedelta(days=1)  # 用timedelta减去1天，得到昨天的日期
        print("开始保存 周线数据任务（当日）...")
        result = async_to_sync(stock_time_trade_dao.save_weekly_time_trade_by_stock_codes)(trade_date=yesterday)
        print(f"保存 周线数据任务（当日） 完成。result: {result}")
    except Exception as e:
        logger.error(f"save_day_data_history_task.执行批量保存任务时发生意外错误: {e}", exc_info=True)
...
def save_month_data_yesterday_task(self):
    """
    ...
    try:
        ...
        result = async_to_sync(stock_time_trade_dao.save_monthly_time_trade_by_stock_codes)(start_date=day_str)
...
def save_cyq_chips_today_batch(self):
    ...
    try:
        today_date = timezone.now().date()
        result = async_to_sync(stock_time_trade_dao.save_all_cyq_chips_history)(trade_date=today_date)
        print(f"保存 每日筹码分布 数据完成。 result: {result} ")
    except Exception as e:
        logger.error(f"save_day_data_history_task.执行批量保存任务时发生意外错误: {e}", exc_info=True)

@celery_app.task(bind=True, name='tasks.tushare.stock_time_trade_tasks.save_cyq_perf_today_batch', queue='SaveHistoryData_TimeTrade', rate_limit='180/m')
def save_cyq_perf_today_batch(self):
    ...
    try:
        today_date = timezone.now().date()
        result = async_to_sync(stock_time_trade_dao.save_all_cyq_perf_history)(trade_date=today_date)
...
def save_cyq_chips_yesterday_batch(self):
    ...
    try:
        ...
        result = async_to_sync(stock_time_trade_dao.save_all_cyq_chips_history)(trade_date=yesterday)
        print(f"保存 每日筹码分布 数据完成。 result: {result} ")
    except Exception as e:
        logger.error(f"save_day_data_history_task.执行批量保存任务时发生意外错误: {e}", exc_info=True)

@celery_app.task(bind=True, name='tasks.tushare.stock_time_trade_tasks.save_cyq_perf_yesterday_batch', queue='SaveHistoryData_TimeTrade', rate_limit='180/m')
def save_cyq_perf_yesterday_batch(self):
    ...
    try:
        today_date = timezone.now().date()
        yesterday = today_date - datetime.timedelta(days=1)  # 用timedelta减去1天，得到昨天的日期时间
        result = async_to_sync(stock_time_trade_dao.save_all_cyq_perf_history)(trade_date=yesterday)
        print(f"保存 每日筹码及胜率 数据完成。 result: {result} ")
    except Exception as e:
        logger.error(f"save_day_data_history_task.执行批量保存任务时发生意外错误: {e}", exc_info=True)
...
#  ================ （本周）分钟数据任务 ================
@celery_app.task(bind=True, name='tasks.tushare.stock_time_trade_tasks.save_minute_data_this_week_batch', queue='SaveHistoryData_TimeTrade')
def save_minute_data_this_week_batch(self, stock_codes: str):
    ...
    try:
        result = async_to_sync(stock_time_trade_dao.save_minute_time_trade_history_by_stock_codes)(stock_codes=stock_codes, start_date_str=this_monday_str, end_date_str=this_friday_str)
...
#  ================ （本周）日线数据任务 ================
@celery_app.task(bind=True, name='tasks.tushare.stock_time_trade_tasks.save_day_data_this_week_batch', queue='SaveHistoryData_TimeTrade')
def save_day_data_this_week_batch(self):
    ...
    stock_time_trade_dao = StockTimeTradeDAO()
    try:
        return_info = async_to_sync(stock_time_trade_dao.save_daily_time_trade_history_by_trade_dates)(start_date=this_monday, end_date=this_friday)
...
def save_stocks_daily_basic_data_this_week_task(self):
    ...
    try:
        ...
        result = async_to_sync(stock_time_trade_dao.save_stock_daily_basic_history_by_trade_date)(start_date=this_monday, end_date=this_friday)
    except Exception as e:
...
def save_cyq_chips_this_week_batch(self, ts_code: str, start_date: datetime.date, end_date: datetime.date):
    ...
    try:
        ...
        stock = async_to_sync(stock_basic_dao.get_stock_by_code)(stock_code=ts_code)
        if not stock:
            logger.warning(f"在数据库中未找到代码为 {ts_code} 的股票，跳过此任务。")
            return
        # [修改] 使用从数据库获取的stock对象进行后续操作
        result = async_to_sync(stock_time_trade_dao.save_cyq_chips_for_stock)(stock=stock, start_date=start_date, end_date=end_date)
...
@celery_app.task(bind=True, name='tasks.tushare.stock_time_trade_tasks.save_cyq_perf_this_week_batch', queue='SaveHistoryData_TimeTrade', rate_limit='180/m')
def save_cyq_perf_this_week_batch(self, start_date: datetime.date, end_date: datetime.date):
    ...
    stock_time_trade_dao = StockTimeTradeDAO()
    try:
        result = async_to_sync(stock_time_trade_dao.save_all_cyq_perf_history)(start_date=start_date, end_date=end_date)
...
def save_cyq_data_this_week_task(self):
    ...
    try:
        ...
        for stock in all_stocks:
            ...
            save_cyq_chips_this_week_batch.s(ts_code=stock.stock_code, start_date=this_monday, end_date=this_friday).set(queue='SaveHistoryData_TimeTrade').apply_async()
        logger.info(f"所有股票的筹码分布任务已分派完毕。")
        # [修改] 将此任务调用移出循环，因为它与单只股票无关，只需执行一次
        logger.info(f"开始分派 （本周）每日筹码及胜率 任务...")
        save_cyq_perf_this_week_batch.s(start_date=this_monday, end_date=this_friday).set(queue='SaveHistoryData_TimeTrade').apply_async()
...
#  ================ 分钟数据任务（历史） ================
@celery_app.task(bind=True, name='tasks.tushare.stock_time_trade_tasks.save_minute_data_history_batch', queue='SaveHistoryData_TimeTrade', rate_limit='480/m')
def save_minute_data_history_batch(self, stock_code: str, time_level: str):
    ...
    stock_time_trade_dao = StockTimeTradeDAO()
    try:
        result = async_to_sync(stock_time_trade_dao.save_minute_time_trade_history_by_stock_code_and_time_level)(stock_code, time_level)
...
#  ================ 日线数据任务（历史） ================
@celery_app.task(bind=True, name='tasks.tushare.stock_time_trade_tasks.save_day_data_history_batch', queue='SaveHistoryData_TimeTrade', rate_limit='180/m')
def save_day_data_history_batch(self, stock_codes: List[str]):
    ...
    stock_time_trade_dao = StockTimeTradeDAO()
    try:
        return_info = async_to_sync(stock_time_trade_dao.save_daily_time_trade_history_by_stock_codes)(stock_codes)
...
#  ================ 每日基本信息数据任务（历史） ================
@celery_app.task(bind=True, name='tasks.tushare.stock_time_trade_tasks.save_basic_data_history_batch', queue='SaveHistoryData_TimeTrade', rate_limit='180/m')
def save_daily_basic_data_history_batch(self, stock_codes: List[str]):
    ...
    stock_time_trade_dao = StockTimeTradeDAO()
    try:
        async_to_sync(stock_time_trade_dao.save_stock_daily_basic_history_by_stock_codes)(stock_codes)
    except Exception as e:
        logger.error(f"执行批量保存任务时发生意外错误: {e}", exc_info=True)
...
#  ================ 历史(周线)数据任务 ================
@celery_app.task(bind=True, name='tasks.tushare.stock_time_trade_tasks.save_week_data_history_batch', queue='SaveHistoryData_TimeTrade', rate_limit='480/m')
def save_week_data_history_batch(self, stock_codes: List[str]):
    ...
    stock_time_trade_dao = StockTimeTradeDAO()
    try:
        async_to_sync(stock_time_trade_dao.save_weekly_time_trade_by_stock_codes)(stock_codes)
    except Exception as e:
        logger.error(f"执行批量保存任务时发生意外错误: {e}", exc_info=True)

# --- 修改后的调度器任务 ---
@celery_app.task(bind=True, name='tasks.tushare.stock_time_trade_tasks.save_stocks_week_data_history_task', queue='celery')
def save_stocks_week_data_history_task(self, batch_size: int = 15): # 限量：单次最大4500行数据
    """
    调度器任务：
    1. 获取自选股和非自选股代码。
    2. 将代码分成批次。
...
#  ================ 历史(月线)数据任务 ================
@celery_app.task(bind=True, name='tasks.tushare.stock_time_trade_tasks.save_month_data_history_batch', queue='SaveHistoryData_TimeTrade', rate_limit='180/m')
def save_month_data_history_batch(self, stock_codes: List[str]):
    """
    从Tushare批量获取实时分钟级交易数据并保存到数据库（异步并发处理）
    Args:
        stock_codes: 股票代码列表
    ...
    stock_time_trade_dao = StockTimeTradeDAO()
    try:
        result = async_to_sync(stock_time_trade_dao.save_monthly_time_trade_by_stock_codes)(stock_codes)
...
def save_stocks_month_data_history_task(self, batch_size: int = 50): # 限量：单次最大6000行数据
    """
    调度器任务：
    1. 获取自选股和非自选股代码。
    2. 将代码分成批次。
    ...
    try:
        ...
        logger.error(f"执行 save_stocks_month_data_history_tasks (调度器模式) 时出错: {e}", exc_info=True)
        return {"status": "error", "message": str(e), "dispatched_batches": 0}
...
def save_cyq_perf_for_stock_task(self, ts_code: str, start_date: datetime.date = None, end_date: datetime.date = None):
    ...
    try:
        ...
        if not stock:
            ...
            return
        # 调用新的DAO方法处理单个股票
        result = async_to_sync(stock_time_trade_dao.save_cyq_perf_for_stock)(stock=stock, start_date=start_date, end_date=end_date)
