...
#  ================ 分钟数据任务（当日） ================
@celery_app.task(bind=True, name='tasks.tushare.stock_time_trade_tasks.save_minute_data_today_batch')
def save_minute_data_today_batch(self, stock_codes: List[str]):
    """
    ...
    try:
        asyncio.run(stock_time_trade_dao.save_minute_time_trade_history_today(stock_codes))
    except Exception as e:
...
def save_stocks_minute_data_today_task(self, batch_size: int = 30): # 限量：单次最大8000行数据
    """
    调度器任务：
    1. 获取自选股和非自选股代码。
    2. 将代码分成批次。
    ...
    try:
        ...
        for i in range(0, total_favorite_stocks, batch_size):
            ...
            if batch:
                ...
                # 使用新的批量任务，并指定队列
                save_minute_data_today_batch.s(batch).set(queue=FAVORITE_SAVE_API_DATA_QUEUE).apply_async()
        ...
        for i in range(0, total_non_favorite_stocks, batch_size):
            ...
            if batch:
                ...
                # 使用新的批量任务，并指定队列
                save_minute_data_today_batch.s(batch).set(queue=STOCKS_SAVE_API_DATA_QUEUE).apply_async()
                total_dispatched_batches += 1
