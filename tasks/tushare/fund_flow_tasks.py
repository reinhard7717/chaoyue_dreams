...
def save_fund_flow_daily_data_ths_history_batch(self, stock_code: str):
    """
    从Tushare批量获取历史日级资金流向数据并保存到数据库（异步并发处理）
    Args:
        stock_codes: 股票代码列表
    ...
    try:
        # 异步获取数据并保存
        asyncio.run(fund_flow_dao.save_history_fund_flow_daily_ths_data_by_stock_code(stock_code))
    except Exception as e:
        logger.error(f"执行批量保存任务时发生意外错误: {e}", exc_info=True)
...
def save_fund_flow_daily_data_ths_history_task(self, batch_size: int = 2): # 限量：单次最大8000行数据
    ...
    try:
        ...
        for stock_code in favorite_codes:
            logger.info(f"创建自选股批次任务 (股票代码: {stock_code})...")
            # 使用新的批量任务，并指定队列
            save_fund_flow_daily_data_ths_history_batch.s(stock_code).set(queue=FAVORITE_SAVE_API_DATA_QUEUE).apply_async()
            total_dispatched_batches += 1
        logger.info(f"已为 {total_favorite_stocks} 个自选股分派了 {total_dispatched_batches} 个批次任务。")
        # 2. 分派非自选股批量任务
        logger.info(f"准备为 {total_non_favorite_stocks} 个非自选股分派批量任务...")
        ...
        for stock_code in non_favorite_codes:
            logger.info(f"创建非自选股批次任务 (股票代码: {stock_code})...")
            # 使用新的批量任务，并指定队列
            save_fund_flow_daily_data_ths_history_batch.s(stock_code).set(queue=STOCKS_SAVE_API_DATA_QUEUE).apply_async()
            total_dispatched_batches += 1
            non_favorite_batches_dispatched += 1
        logger.info(f"已为 {total_non_favorite_stocks} 个非自选股分派了 {non_favorite_batches_dispatched} 个批次任务。")
        logger.info(f"任务结束: save_stocks_realtime_min_data_task (调度器模式) - 共分派 {total_dispatched_batches} 个批量任务")
...
#  ================ （历史）日级资金流向数据 - 东方财富 ================
@celery_app.task(bind=True, name='tasks.tushare.fund_flow_tasks.save_fund_flow_daily_data_dc_history_batch')
def save_fund_flow_daily_data_dc_history_batch(self, stock_code: str):
    """
    从Tushare批量获取历史日级资金流向数据并保存到数据库（异步并发处理）
    Args:
        stock_codes: 股票代码列表
    ...
    try:
        # 异步获取数据并保存
        asyncio.run(fund_flow_dao.save_history_fund_flow_daily_dc_data_stock_code(stock_code))
    except Exception as e:
        logger.error(f"执行批量保存任务时发生意外错误: {e}", exc_info=True)

# --- 修改后的调度器任务 ---
@celery_app.task(bind=True, name='tasks.tushare.fund_flow_tasks.save_fund_flow_daily_data_dc_history_task')
def save_fund_flow_daily_data_dc_history_task(self, batch_size: int = 2): # 限量：单次最大8000行数据
    """
    调度器任务：
    1. 获取自选股和非自选股代码。
    2. 将代码分成批次。
    ...
    try:
        ...
        for stock_code in favorite_codes:
            logger.info(f"创建自选股批次任务 (股票代码: {stock_code})...")
            # 使用新的批量任务，并指定队列
            save_fund_flow_daily_data_dc_history_batch.s(stock_code).set(queue=FAVORITE_SAVE_API_DATA_QUEUE).apply_async()
            total_dispatched_batches += 1
        logger.info(f"已为 {total_favorite_stocks} 个自选股分派了 {total_dispatched_batches} 个批次任务。")
        # 2. 分派非自选股批量任务
        logger.info(f"准备为 {total_non_favorite_stocks} 个非自选股分派批量任务...")
        ...
        for stock_code in non_favorite_codes:
            logger.info(f"创建非自选股批次任务 (股票代码: {stock_code})...")
            # 使用新的批量任务，并指定队列
            save_fund_flow_daily_data_dc_history_batch.s(stock_code).set(queue=STOCKS_SAVE_API_DATA_QUEUE).apply_async()
            total_dispatched_batches += 1
            non_favorite_batches_dispatched += 1
        logger.info(f"已为 {total_non_favorite_stocks} 个非自选股分派了 {non_favorite_batches_dispatched} 个批次任务。")
        logger.info(f"任务结束: save_stocks_realtime_min_data_task (调度器模式) - 共分派 {total_dispatched_batches} 个批量任务")
