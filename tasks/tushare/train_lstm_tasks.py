...
def batch_prepare_lstm_data(self, stock_code: str, params_file: str = "strategies/indicator_parameters.json", model_dir="models", base_bars: int = 10000):
    ...
    logger.info(f"[{stock_code}] 原始数据获取完成，共 {len(data_df)} 条。开始准备 LSTM 数据...")
...
# 修改调度器任务，创建任务链
@celery_app.task(bind=True, name='tasks.tushare.train_lstm_tasks.prepare_lstm_data_task')
def prepare_lstm_data_task(self, base_bars_to_request: int = 8000):
    """
    调度器任务：
    1. 获取股票代码列表。
    ...
    try:
        ...
        for stock in all_stocks:
            ...
            # 定义模型训练任务签名
            train_task = batch_prepare_lstm_data.s( stock_code=stock_code).set(queue="Train_LSTM").apply_async() # 可以指定模型训练队列
