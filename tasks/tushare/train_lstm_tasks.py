...
@celery_app.task(bind=True, name='tasks.train_lstm_tasks.batch_train_following_strategy_lstm')
def batch_train_following_strategy_lstm(stock_code, params_file: str = "strategies/indicator_parameters.json", model_dir="models"):
    """
    ...
    try:
        ...
        print(f"[{stock_info}] 调用 prepare_strategy_dataframe 准备数据时出错: {prep_err}")
        return

    if data_df is None or data_df.empty:
        print(f"[{stock_info}] 未能准备足够的数据 (prepare_strategy_dataframe 返回空)。")
        return
...
def train_lstm_trend_following_strategy_task(self): # 最大循环10万个，每310个一组循环一次是99510个
    ...
    """
    logger.info(f"任务启动: train_lstm_trend_following_strategy_task (调度器模式) - 获取股票列表并分派批量任务 (批次大小: {batch_size})")
    try:
        total_dispatched_batches = 0
        stock_basic_dao = StockBasicInfoDao()
        all_stocks = asyncio.run(stock_basic_dao.get_stock_list())
        for stock_dict in all_stocks:
            stock_code = stock_dict.get("stock_code")
            logger.info(f"创建自选股批次任务 (大小: {len(batch_codes)})...")
            # 使用新的批量任务，并指定队列
            batch_train_following_strategy_lstm.s(stock_code=stock_code).set(queue="Train_LSTM").apply_async()
