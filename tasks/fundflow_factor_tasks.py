...
from stock_models.fund_flow import (
    FundFlowDailyBase, FundFlowDailyTHSBase, FundFlowDailyDCBase
)
...
def schedule_fundflow_factors_calculation(self, start_date_str: str = None, 
                                         batch_size: int = 100, 
...
def calculate_fundflow_factors_for_stock(self, stock_code: str, 
...
def get_trade_dates_for_stock(stock_code: str, 
                             start_date: date,
                             incremental: bool,
                             factor_model) -> List[date]:
    """
    获取需要计算的交易日列表
...
def calculate_factor_batch(stock_code: str, trade_dates: List[date], 
                          factor_model) -> Tuple[int, List[date]]:
    """
    批量计算资金流向因子
...
def calculate_single_date_factor(stock_code: str, trade_date: date, 
                               factor_model) -> bool:
    """
    计算单个日期的资金流向因子
    """
...
def get_single_date_flow_data(stock_code: str, trade_date: date, 
                             stock_info: StockInfo = None) -> Optional[Dict]:
    """获取单个日期的资金流向数据"""
    try:
...
@celery_app.task
def test_fundflow_factor_calculation(stock_code: str = '000001.SZ', 
                                    test_date_str: str = None):
    """
