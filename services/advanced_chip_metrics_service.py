...
from stock_models.time_trade import StockDailyBasic, StockCyqPerf, BaseAdvancedChipMetrics
...
class AdvancedChipMetricsService:
    ...
    async def _load_minute_data_for_range(self, stock_info: StockInfo, start_date: pd.Timestamp, end_date: pd.Timestamp):
        ...
        def get_data(model, stock_pk, start_dt, end_dt):
            qs = model.objects.filter(stock_id=stock_pk, trade_time__gte=start_dt, trade_time__lt=end_dt).values('trade_time', 'amount', 'vol', 'open', 'close', 'high', 'low')
    ...
    def _calculate_derivatives(self, consensus_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in CORE_METRICS_TO_DERIVE:
            if col in final_df.columns and col not in SLOPE_ACCEL_EXCLUSIONS and col not in BaseAdvancedChipMetrics.BOOLEAN_FIELDS:
                source_series = pd.to_numeric(final_df[col], errors='coerce')
