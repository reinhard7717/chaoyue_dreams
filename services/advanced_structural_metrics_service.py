...
class AdvancedStructuralMetricsService:
    ...
    async def run_precomputation(self, stock_code: str, is_incremental: bool, start_date_str: str = None):
        """
        【V1.0 · 新增】高级结构与行为指标预计算总指挥
        - 核心职责: 编排整个预计算流程，包括上下文初始化、数据加载、核心计算、衍生计算和数据存储。
        """
        ...
        for i in range(0, len(dates_to_process), CHUNK_SIZE):
            ...
            # 步骤 4.1: 为当前块加载所需的分钟级原始数据
            minute_df_chunk = await self._load_minute_data_for_chunk(stock_info, chunk_dates)
            if minute_df_chunk is None or minute_df_chunk.empty:
                continue
            
            # 步骤 4.2: 调用核心锻造引擎，计算高保真结构指标
            chunk_new_metrics_df = self._forge_advanced_structural_metrics(minute_df_chunk)
    ...
    async def _load_minute_data_for_chunk(self, stock_info: StockInfo, date_index: pd.DatetimeIndex) -> pd.DataFrame:
        """
        【V1.0 · 新增】为给定的日期块加载所有相关的分钟级数据。
        """
    ...
    def _forge_advanced_structural_metrics(self, minute_df: pd.DataFrame) -> pd.DataFrame:
        """
        【V2.0 · 结构升维版】高级结构与行为指标锻造核心引擎
        - 核心升级: 全面引入日内价值区间、路径分析、订单流足迹、动能分析等高保真指标的计算。
        """
        if minute_df.empty:
            return pd.DataFrame()
            
        minute_df['date'] = minute_df['trade_time'].dt.date
        minute_df_grouped = minute_df.groupby('date')
        
        daily_metrics = []
        for date, group in minute_df_grouped:
