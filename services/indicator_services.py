...
class IndicatorService:
    ...
    async def prepare_strategy_dataframe(self, stock_code: str, params_file: str, base_needed_bars: Optional[int] = None) -> Optional[Tuple[pd.DataFrame, List[Dict[str, Any]]]]:
        ...
        """
        # 检查 pandas_ta 是否已加载
        # 注意: 此处假设 ta 是在类外部或模块级别导入的。如果不是，请取消下面 pandas_ta 导入的注释。
        # import pandas_ta as ta
        if 'ta' not in globals() or ta is None: # 更安全的检查方式，或者直接使用 self.ta (如果它是类属性)
        ...
        try:
            ...
            # print(f"[{stock_code}] Debug: 加载策略参数成功，部分参数: base_scoring={params.get('base_scoring', {})}, feature_engineering_params={params.get('feature_engineering_params', {})}") # 调试输出：打印部分加载的参数
        except Exception as e:
            # 记录加载或解析失败的错误
        ...
        all_time_levels_needed: Set[str] = set() # 存储所有需要的时间级别，使用集合避免重复
        indicator_configs: List[Dict[str, Any]] = [] # 存储每个指标的计算配置
        # 辅助函数：用于简化从参数中提取指标配置并添加到 indicator_configs 列表的过程
        def _add_indicator_config(
            name: str, # 指标名称
            func: Callable, # 计算该指标的异步函数引用
            param_block_key: Optional[str], # 参数块的键名
            params_dict: Dict, # 传递给计算函数的参数字典
            applicable_tfs: Union[str, List[str]], # 适用该指标的时间级别
            param_override_key: Optional[str] = None # 参数覆盖键名
        ):
            tfs = [applicable_tfs] if isinstance(applicable_tfs, str) else applicable_tfs # 确保时间级别是列表
            all_time_levels_needed.update(tfs) # 添加到总的时间级别需求集合
            indicator_configs.append({ # 添加指标配置到列表
                'name': name,
                'func': func,
                'params': params_dict,
                'timeframes': tfs,
        ...
        def _get_indicator_params(param_block: Dict, default_params: Dict, param_override_key: Optional[str] = None) -> Dict:
             """从参数块中提取并合并单个指标的参数。"""
             indi_specific_params_json = param_block.get(param_override_key, param_block) if param_override_key else param_block # 获取指标特定参数或整个块
             final_calc_params = default_params.copy() # 复制默认参数
             for k, v_json in indi_specific_params_json.items(): # 遍历JSON中的参数
                 if k in final_calc_params: # 如果键存在于默认参数中，则覆盖
                      final_calc_params[k] = v_json
             return final_calc_params
        # --- 从参数文件动态构建指标计算列表 ---
        bs_params = params.get('base_scoring', {}) # 获取基础评分相关的参数
        bs_timeframes = bs_params.get('timeframes', ['5', '15', '30', '60', 'D']) # 基础评分默认使用的时间级别
        all_time_levels_needed.update(bs_timeframes) # 将基础评分时间级别添加到总所需时间级别集合中
        # 定义常用指标的默认参数
        ...
        # --- 注册基础评分指标的计算配置 ---
        for indi_key in bs_params.get('score_indicators', []): # 遍历 base_scoring.score_indicators 列表
            if indi_key == 'macd': # MACD 指标
                macd_calc_params = { # 从 JSON 或默认值获取 MACD 参数
            ...
            elif indi_key == 'rsi': # RSI 指标
                calc_params = {'period': bs_params.get('rsi_period', default_rsi_p['period'])} # 获取 RSI 周期
                _add_indicator_config('RSI', self.calculate_rsi, 'base_scoring', calc_params, bs_timeframes)
            elif indi_key == 'kdj': # KDJ 指标
                calc_params = { # 获取 KDJ 参数
            ...
            elif indi_key == 'boll': # BOLL 指标
                calc_params = { # 获取 BOLL 参数
            ...
            elif indi_key == 'sar': # SAR 指标
                calc_params = { # 获取 SAR 参数
            ...
            elif indi_key == 'ema': # EMA 指标 (如果在评分指标中配置)
            ...
            elif indi_key == 'sma': # SMA 指标 (如果在评分指标中配置)
        ...
        target_vol_ana_tfs = list(set(vol_ana_tfs_vc) | set(ia_tfs) | set(bs_timeframes)) # 合并所有相关时间级别
        all_time_levels_needed.update(target_vol_ana_tfs)
        if vc_params.get('enabled', False) or ia_params.get('calculate_amt_ma', False): # AMT_MA 计算
        ...
        if vc_params.get('enabled', False) or ia_params.get('calculate_cmf', False): # CMF 计算
        ...
        if ia_params.get('calculate_vol_ma', False): # VOL_MA 计算
        ...
        ia_timeframes = ia_params.get('timeframes', bs_timeframes) # 获取指标分析参数的时间级别
        all_time_levels_needed.update(ia_timeframes)
        if ia_params.get('calculate_stoch', False): # STOCH 计算
        ...
        if ia_params.get('calculate_pivot_points', False): # Pivot Points 计算
            pivot_calc_params = _get_indicator_params(ia_params, {}, param_override_key='pivot_params')
            _add_indicator_config('PivotPoints', self.calculate_pivot_points, 'indicator_analysis_params', pivot_calc_params, ['D']) # 通常基于日线
            all_time_levels_needed.add('D')
        # --- 注册特征工程指标的计算配置 ---
        fe_timeframes_cfg = fe_params.get('apply_on_timeframes', bs_timeframes) # 获取特征工程应用的时间级别
        ...
        if fe_params.get('calculate_atr', False): # ATR 计算
        ...
        if fe_params.get('calculate_hv', False): # 历史波动率 (HV) 计算
        ...
        if fe_params.get('calculate_kc', False): # 肯特纳通道 (KC) 计算
        ...
        if fe_params.get('calculate_mom', False): # 动量 (MOM) 计算
        ...
        if fe_params.get('calculate_willr', False): # Williams %R (WILLR) 计算
        ...
        if fe_params.get('calculate_vroc', False): # 成交量变化率 (VROC) 计算
        ...
        if fe_params.get('calculate_aroc', False): # 成交额变化率 (AROC) 计算
        ...
        for ma_type, ma_func in [('EMA', self.calculate_ema), ('SMA', self.calculate_sma)]:
            ma_periods = fe_params.get(f'{ma_type.lower()}_periods', []) # 获取均线周期列表
            if ma_periods: # 如果配置了周期列表
                for p_val in ma_periods: # 遍历每个周期
                     if isinstance(p_val, int) and p_val > 0: # 确保周期是有效的正整数
                         ma_calc_params = {'period': p_val}
                         _add_indicator_config(ma_type, ma_func, 'feature_engineering_params', ma_calc_params, fe_timeframes)
            elif fe_params.get(f'calculate_{ma_type.lower()}', False): # 如果没有指定列表，但calculate标志为True
                 ma_p = fe_params.get(f'{ma_type.lower()}_period', default_sma_ema_p['period']) # 使用单个周期或默认周期
                 ma_calc_params = {'period': ma_p}
                 _add_indicator_config(ma_type, ma_func, 'feature_engineering_params', ma_calc_params, fe_timeframes)
        # OBV 计算，确保只添加一次
        if not any(conf['name'] == 'OBV' for conf in indicator_configs):
            _add_indicator_config('OBV', self.calculate_obv, None, {}, list(all_time_levels_needed)) # OBV 无参数
        focus_tf = params.get('trend_following_params', {}).get('focus_timeframe', '30') # 获取关注的时间级别
        ...
        # --- 确定最小时间级别 ---
        min_time_level = None # 初始化最小时间级别
        min_tf_minutes = float('inf') # 初始化最小时间级别的分钟数
        if not all_time_levels_needed: # 如果没有任何时间级别被识别
            logger.error(f"[{stock_code}] 未能从参数文件中确定任何需要的时间级别。")
            return None
        for tf_str_loop in all_time_levels_needed: # 遍历所有需要的时间级别
            minutes = self._get_timeframe_in_minutes(tf_str_loop) # 获取时间级别的分钟数
            if minutes is not None and minutes < min_tf_minutes: # 如果更小，则更新
                 min_tf_minutes = minutes
                 min_time_level = tf_str_loop
        if min_time_level is None and bs_timeframes: # 如果无法确定，回退
             min_time_level = bs_timeframes[0]
             print(f"[{stock_code}] Debug: 无法精确确定最小时间级别 (按分钟)，回退使用基础时间级别列表的第一个: {min_time_level}")
        elif min_time_level is None: # 仍然无法确定
        ...
        global_max_lookback = 0 # 初始化全局最大回看期
        unique_configs_for_lookback = {} # 用于去重的配置字典
        for config in indicator_configs: # 遍历所有指标配置
            if config.get('param_block_key') is None and config['name'] != 'OBV': # 跳过无参数块的非OBV配置
                 continue
            params_hashable = tuple(sorted(config['params'].items())) # 参数可哈希化
            key = (config['name'], params_hashable) # 构建唯一键
            if key not in unique_configs_for_lookback: # 如果是新的唯一配置
                 unique_configs_for_lookback[key] = config
        print(f"[{stock_code}] Debug: 用于回看期计算的唯一指标配置数量: {len(unique_configs_for_lookback)}")
        for config in unique_configs_for_lookback.values(): # 遍历唯一配置
            current_max_period = 0 # 当前指标的最大周期
            ...
            for p_key in period_keys: # 遍历可能的周期参数键名
                if p_key == 'atr_multiplier': # atr_multiplier 不是回看周期
                    continue
                if p_key in config['params'] and isinstance(config['params'][p_key], (int, float)): # 获取参数值
            ...
            if config['name'] == 'DMI' and 'period' in config['params']:
                current_max_period = max(current_max_period, int(config['params']['period'] * 2.5 + 10)) # DMI/ADX 需要更多数据
            if config['name'] == 'KC':
                 p_kc = config['params']
            ...
            if config['name'] in ['AMT_MA', 'VOL_MA', 'MOM', 'WILLR', 'VROC', 'AROC', 'RSI', 'CCI', 'MFI', 'ROC', 'ATR', 'SAR']:
                 current_max_period = max(current_max_period,
                                           ...
                                           config['params'].get('k_period', 0), # KDJ, STOCH
                                           config['params'].get('atr_period', 0)) # ATR
            if config['name'] == 'KDJ':
                 p_kdj = config['params']
                 current_max_period = max(current_max_period, p_kdj.get('period', 0), p_kdj.get('signal_period', 0), p_kdj.get('smooth_k_period', 0))
            global_max_lookback = max(global_max_lookback, current_max_period) # 更新全局最大回看期
        global_max_lookback += 100 # 添加固定缓冲期
        logger.info(f"[{stock_code}] 动态计算的全局指标最大回看期 (含缓冲): {global_max_lookback}")
        # 3. 并行获取原始 OHLCV 数据
        ohlcv_tasks = {} # 存储异步数据获取任务
        lstm_window_size = params.get('lstm_training_config',{}).get('lstm_window_size', 60) # 获取 LSTM 窗口大小
        effective_base_needed_bars = base_needed_bars if base_needed_bars is not None else \
                                     lstm_window_size + global_max_lookback + 500 # 确定基础所需数据条数
        for tf_fetch in all_time_levels_needed: # 为每个所需时间级别创建数据获取任务
            needed_bars_for_tf = self._calculate_needed_bars_for_tf( # 估算需获取的原始数据条数
            ...
            ohlcv_tasks[tf_fetch] = self._get_ohlcv_data(stock_code, tf_fetch, needed_bars=needed_bars_for_tf) # 创建异步获取任务
        ohlcv_results = await asyncio.gather(*ohlcv_tasks.values()) # 并行执行所有数据获取任务
        raw_ohlcv_dfs = dict(zip(all_time_levels_needed, ohlcv_results)) # 将结果按时间级别字典化
        # print(f"[{stock_code}] Debug: 原始 OHLCV 数据获取结果:") # 调试输出
        for tf_debug, df_debug in raw_ohlcv_dfs.items(): # 调试输出原始数据状态
        ...
        min_usable_bars = math.ceil(effective_base_needed_bars * 0.6) # 定义最小可用数据条数门槛
        for tf_resample, raw_df in raw_ohlcv_dfs.items(): # 遍历所有获取到的原始数据
            if raw_df is None or raw_df.empty: # 检查原始数据是否有效
                logger.warning(f"[{stock_code}] 时间级别 {tf_resample} 没有获取到原始数据，跳过重采样。")
                continue
            # 调用 _resample_and_clean_dataframe 进行重采样、初步填充并添加后缀
            resampled_df_processed = self._resample_and_clean_dataframe(raw_df, tf_resample, min_periods=1, fill_method='ffill')
            if resampled_df_processed is None or resampled_df_processed.empty: # 检查重采样后数据是否有效
                logger.warning(f"[{stock_code}] 时间级别 {tf_resample} 重采样后数据为空，跳过。")
                continue
            if tf_resample == min_time_level and len(resampled_df_processed) < min_usable_bars: # 对最小时间级别的数据量进行硬性检查
            ...
            if len(resampled_df_processed) < global_max_lookback * 0.5: # 如果数据量显著少于全局最大回看期
            ...
            # print(f"[{stock_code}] Debug: TF {tf_resample} 重采样并重命名后的列: {resampled_df_processed.columns.tolist()[:20]}...") # 调试输出
            resampled_ohlcv_dfs[tf_resample] = resampled_df_processed # 存储处理后的 DataFrame
        if min_time_level not in resampled_ohlcv_dfs or resampled_ohlcv_dfs[min_time_level].empty: # 最终检查最小时间级别数据
             logger.error(f"[{stock_code}] 最小时间级别 {min_time_level} 的重采样 OHLCV 数据不可用，无法进行合并。")
             return None
        base_index = resampled_ohlcv_dfs[min_time_level].index # 使用最小时间级别数据索引作为基准
        ...
        async def _calculate_single_indicator_async(tf_calc: str, base_df_with_suffix: pd.DataFrame, config_item: Dict) -> Optional[Tuple[str, pd.DataFrame]]:
            ...
            if base_df_with_suffix is None or base_df_with_suffix.empty: # 检查基础数据是否有效
                print(f"[{stock_code}] Debug: TF {tf_calc}: 基础OHLCV数据为空，无法计算指标 {config_item['name']}")
                return None
            df_for_ta = base_df_with_suffix.copy() # 复制 DataFrame
            ohlcv_map_to_std = { # 定义标准列名映射
            ...
            actual_rename_map_to_std = {k: v for k, v in ohlcv_map_to_std.items() if k in df_for_ta.columns} # 过滤不存在的列
            df_for_ta.rename(columns=actual_rename_map_to_std, inplace=True) # 应用临时重命名
            required_cols_for_func = set(['high', 'low', 'close']) # 默认需要 HLC
            if config_item['name'] in ['MFI', 'OBV', 'VWAP', 'CMF', 'VOL_MA', 'ADL']: # 需要成交量的指标
                required_cols_for_func.add('volume')
            if config_item['name'] in ['AMT_MA', 'AROC']: # 需要成交额的指标
                required_cols_for_func.add('amount')
            if not all(col in df_for_ta.columns for col in required_cols_for_func): # 检查是否包含所有必需列
                ...
                return None
            try:
                func_params_to_pass = config_item['params'].copy() # 获取参数副本
                indicator_result_df = await config_item['func'](df_for_ta, **func_params_to_pass) # 调用指标计算函数
                if indicator_result_df is None: # 检查计算结果
                     ...
                     return None
                if indicator_result_df.empty: # 检查计算结果是否为空
                ...
                if not isinstance(indicator_result_df, pd.DataFrame): # 确保返回的是 DataFrame
                    logger.warning(f"[{stock_code}] TF {tf_calc}: 指标 {config_item['name']} 计算函数未返回DataFrame (返回类型: {type(indicator_result_df)})。尝试转换。")
                    if isinstance(indicator_result_df, pd.Series): # 如果是 Series，转换为 DataFrame
                ...
                return (tf_calc, result_renamed_df) # 返回时间级别和带有后缀的结果 DataFrame
            except Exception as e_calc: # 捕获计算过程中的异常
        ...
        calculated_indicators_by_tf = defaultdict(list) # 按时间级别对计算结果进行分组存储
        for res_tuple_item in calculated_results_tuples: # 遍历计算结果
            if isinstance(res_tuple_item, tuple) and len(res_tuple_item) == 2: # 检查任务结果是否成功
                tf_res, indi_df_res = res_tuple_item
                if indi_df_res is not None and not indi_df_res.empty: # 检查返回的 DataFrame 是否有效
                    calculated_indicators_by_tf[tf_res].append(indi_df_res) # 添加到对应时间级别列表
                    # print(f"[{stock_code}] Debug: TF {tf_res}: 成功添加指标 DataFrame 到分组列表，列: {indi_df_res.columns.tolist()}") # 调试输出 (可能刷屏)
                else:
                    pass # 计算结果为空的情况已在 _calculate_single_indicator_async 中记录
            elif isinstance(res_tuple_item, Exception): # 记录计算任务中的异常
        ...
        final_df = resampled_ohlcv_dfs.get(min_time_level) # 从最小时间级别的数据开始
        if final_df is None or final_df.empty: # 检查基础数据是否有效
             logger.error(f"[{stock_code}] 最小时间级别 {min_time_level} 的重采样 OHLCV 数据不可用，无法进行合并。")
             return None
        ...
        for tf_merge, df_to_merge in resampled_ohlcv_dfs.items():
             if tf_merge != min_time_level and df_to_merge is not None and not df_to_merge.empty:
                  final_df = pd.merge(final_df, df_to_merge, left_index=True, right_index=True, how='left') # 左连接合并
        ...
        for tf_indi, indi_dfs_list in calculated_indicators_by_tf.items():
             if indi_dfs_list: # 如果该时间级别有计算出的指标
                  merged_indi_df_for_tf = pd.concat(indi_dfs_list, axis=1) # 合并同一时间级别的所有指标
                  if not merged_indi_df_for_tf.empty:
                       final_df = pd.merge(final_df, merged_indi_df_for_tf, left_index=True, right_index=True, how='left') # 左连接合并
        ...
        if final_df is None or final_df.empty: # 检查合并后的 DataFrame 是否有效
        ...
        # --- 修改开始: 获取用于滞后/滚动特征的实际指标参数 ---
        # 从 base_scoring 获取实际使用的参数，用于构建正确的指标列名
        actual_rsi_period = bs_params.get('rsi_period', default_rsi_p['period']) # 获取实际RSI周期
        actual_macd_fast = bs_params.get('macd_fast', default_macd_p['period_fast']) # 获取实际MACD快周期
        actual_macd_slow = bs_params.get('macd_slow', default_macd_p['period_slow']) # 获取实际MACD慢周期
        actual_macd_signal = bs_params.get('macd_signal', default_macd_p['signal_period']) # 获取实际MACD信号周期
        # --- 修改结束: 获取用于滞后/滚动特征的实际指标参数 ---
        ...
        # 8.1 计算相对强度/超额收益
        rs_config = fe_config.get('relative_strength', {}) # 获取相对强度配置
        if rs_config.get('enabled', False): # 如果启用
             # 动态确定相对强度的基准代码列表
             ths_indexs_for_rs = await self.industry_dao.get_stock_ths_indices(stock_code) # 获取股票所属同花顺板块
             if ths_indexs_for_rs is None:
                  logger.warning(f"[{stock_code}] 无法获取股票 {stock_code} 的同花顺板块信息。相对强度计算将跳过。")
                  ths_codes_for_rs = []
             else:
                ths_codes_for_rs = [m.ths_index.ts_code for m in ths_indexs_for_rs if m.ths_index] # 提取板块代码
        ...
        # 8.2 添加滞后特征
        lag_config = fe_config.get('lagged_features', {}) # 获取滞后特征配置
        if lag_config.get('enabled', False): # 如果启用
             columns_to_lag_from_json = lag_config.get('columns_to_lag', []) # 从JSON获取要滞后的列名模板
             lags = lag_config.get('lags', [1, 2, 3]) # 获取滞后阶数
             if columns_to_lag_from_json and lags: # 如果有列和阶数配置
                  logger.info(f"[{stock_code}] 开始添加滞后特征...")
                  for tf_apply in apply_on_tfs: # 在每个适用时间级别上应用
                       actual_cols_for_lagging = [] # 存储实际用于滞后的列名
                       for col_template_from_json in columns_to_lag_from_json: # 遍历JSON中的列名模板
                            effective_base_name = col_template_from_json # 默认使用JSON中的原始名称
                            # --- 根据实际计算参数调整指标基础名称 ---
                            if col_template_from_json.startswith("RSI"): # 如果是RSI指标
                                effective_base_name = f"RSI_{actual_rsi_period}" # 使用实际RSI周期构建名称
                            elif col_template_from_json.startswith("MACD_") and \
                                 not col_template_from_json.startswith("MACDh_") and \
                                 not col_template_from_json.startswith("MACDs_"): # 如果是MACD值指标
                                effective_base_name = f"MACD_{actual_macd_fast}_{actual_macd_slow}_{actual_macd_signal}" # 使用实际MACD参数
                            elif col_template_from_json.startswith("MACDh_"): # 如果是MACD柱状图(histogram)指标
                                effective_base_name = f"MACDh_{actual_macd_fast}_{actual_macd_slow}_{actual_macd_signal}" # 使用实际MACD参数
                            elif col_template_from_json.startswith("MACDs_"): # 如果是MACD信号线(signal)指标
                                effective_base_name = f"MACDs_{actual_macd_fast}_{actual_macd_slow}_{actual_macd_signal}" # 使用实际MACD参数
                            # 对于 "close", "volume" 等非参数化指标，effective_base_name 将保持原样
                            col_with_suffix = f"{effective_base_name}_{tf_apply}" # 构建带时间后缀的列名
                            if col_with_suffix in final_df.columns: # 如果带后缀的列存在
                                 actual_cols_for_lagging.append(col_with_suffix)
                            else: # 如果带后缀的列不存在
                                 if effective_base_name in final_df.columns: # 尝试查找不带后缀的列 (如外部特征)
                       ...
                       if actual_cols_for_lagging: # 如果找到了有效的列进行滞后
        ...
        # 8.3 添加滚动统计特征
        roll_config = fe_config.get('rolling_features', {}) # 获取滚动统计特征配置
        if roll_config.get('enabled', False): # 如果启用
             columns_to_roll_from_json = roll_config.get('columns_to_roll', []) # 从JSON获取要滚动的列名模板
             windows = roll_config.get('windows', [5, 10, 20]) # 获取滚动窗口大小
             stats = roll_config.get('stats', ["mean", "std"]) # 获取统计函数
             if columns_to_roll_from_json and windows and stats: # 如果有列、窗口和统计函数配置
                  logger.info(f"[{stock_code}] 开始添加滚动统计特征...")
                  for tf_apply in apply_on_tfs: # 在每个适用时间级别上应用
                       actual_cols_for_rolling = [] # 存储实际用于滚动的列名
                       for col_template_from_json in columns_to_roll_from_json: # 遍历JSON中的列名模板
                            effective_base_name = col_template_from_json # 默认使用JSON中的原始名称
                            # --- 根据实际计算参数调整指标基础名称 ---
                            ...
                            if col_with_suffix in final_df.columns: # 如果带后缀的列存在
                                 ...
                                 if effective_base_name in final_df.columns: # 尝试查找不带后缀的列
                       ...
                       if actual_cols_for_rolling: # 如果找到了有效的列进行滚动统计
        ...
        # 9. 最终缺失值填充
        original_nan_count = final_df.isnull().sum().sum() # 记录填充前 NaN 数量
        final_df.ffill(inplace=True) # 前向填充
        final_df.bfill(inplace=True) # 后向填充
        nan_count_after_fill = final_df.isnull().sum().sum() # 记录填充后 NaN 数量
        if nan_count_after_fill > 0: # 如果仍有缺失值
