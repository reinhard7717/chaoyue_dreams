...
def calculate_chip_health_score(row: pd.Series) -> float:
    ...
    if pd.notna(conc_90):
        ...
        # [修改] 将 decimal.Decimal 类型的 conc_90 转换为 float 类型，以避免与 float 类型的 0.3 运算时发生 TypeError
    ...
    if pd.notna(conc_slope):
        # [修改] 将 conc_slope 转换为 float 类型，以确保与 0 的比较和后续运算的类型一致性
    ...
    if pd.notna(profit_margin):
        # 每1%的安全垫加1分，最多加25分
        # [修改] 将 profit_margin 转换为 float 类型，以确保与整数/浮点数运算的类型安全
    ...
    if pd.notna(price_ratio):
        # [修改] 将 price_ratio 转换为 float 类型，以确保与浮点数 1.05 比较的类型安全
