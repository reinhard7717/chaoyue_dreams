...
class RealtimeServices:
    ...
    async def process_all_stocks_intraday_data(self, stock_codes: List[str], time_level: str, trade_date: str):
        ...
        for stock_code in stock_codes:
            ...
            if data_tuple is None:
                logger.warning(f"股票 {stock_code} 在批量获取结果中没有数据，已跳过。")
                continue

            # 将预加载的 Ticks 和 Level5 DataFrame 作为参数传递
            task = asyncio.create_task(
                self.process_stock_intraday_data(
                    stock_code, time_level, trade_date, 
                    preloaded_ticks=data_tuple[0], 
                    preloaded_level5=data_tuple[1]
                )
            )
            tasks.append(task)
    ...
    async def process_stock_intraday_data(self, stock_code: str, time_level: str, trade_date: str, preloaded_ticks: Optional[pd.DataFrame], preloaded_level5: Optional[pd.DataFrame]):
        """
        【V4.4 - 单股处理器】接收预加载的 Ticks 和 Level5 数据。
        """
        try:
            # 调用核心准备函数，并将预加载的数据全部传入
            intraday_df = await self.prepare_intraday_data(
                stock_code, time_level, trade_date, 
                preloaded_ticks=preloaded_ticks, 
                preloaded_level5=preloaded_level5
            )
