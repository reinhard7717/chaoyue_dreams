...
class ChipFeatureCalculator:
    ...
    def _compute_contextual_action_metrics(self, context: dict) -> dict:
        ...
        if not above_peak_zone_df.empty:
            ...
            metrics['distribution_at_peak_intensity'] = np.clip(focus_component * intent_component * (1 + outcome_component), 0, 1) # 修改：移除 *100，归一化到 [0, 1]
        # 2. 计算主峰区吸筹烈度
        if not below_peak_zone_df.empty:
            ...
            metrics['absorption_at_peak_intensity'] = np.clip(focus_component * intent_component * (1 + outcome_component), 0, 1) # 修改：移除 *100，归一化到 [0, 1]
        # 3. 计算突破主峰质量
        ...
        if not breakthrough_event_df.empty:
            ...
            metrics['breakthrough_of_peak_quality'] = np.clip(magnitude_component * conviction_component * efficiency_component, 0, 1) # 修改：移除 *100，归一化到 [0, 1]
        # 4. 计算防守主峰质量
        ...
        if day_low >= peak_zone_lower:
            ...
            if not recovery_df.empty and pd.notna(day_close):
                ...
                metrics['defense_of_peak_quality'] = np.clip(recovery_magnitude * recovery_conviction, 0, 1) # 修改：移除 *100，归一化到 [0, 1]
        if should_probe:
