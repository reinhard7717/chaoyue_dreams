...
class AdvancedFundFlowMetricsService:
    ...
    @staticmethod
    def _identify_trade_participants(hf_analysis_df: pd.DataFrame) -> Tuple[pd.Series, pd.Series]:
        """
    ...
    def _calculate_vpoc_metrics(context: dict) -> dict:
            ...
            if 'main_force_net_vol' in intraday_data.columns and 'minute_vwap' in intraday_data.columns and 'vol_shares' in intraday_data.columns:
                ...
                if not vp_global.empty:
                    ...
                    if not peak_zone_df.empty:
                        ...
                        mf_buy_vol_on_peak_fallback = peak_zone_df['main_force_buy_vol'].sum()
                        mf_sell_vol_on_peak_fallback = peak_zone_df['main_force_sell_vol'].sum()
                        if daily_total_amount > 0:
                            metrics['main_force_on_peak_buy_flow'] = np.tanh((mf_buy_vol_on_peak_fallback * global_vpoc_price) / daily_total_amount)
                            metrics['main_force_on_peak_sell_flow'] = np.tanh((mf_sell_vol_on_peak_fallback * global_vpoc_price) / daily_total_amount)
        ...
        else: # Fallback if global_vpoc_interval is None
            if 'main_force_net_vol' in intraday_data.columns and 'minute_vwap' in intraday_data.columns and 'vol_shares' in intraday_data.columns:
                vp_global = intraday_data.groupby(pd.cut(intraday_data['minute_vwap'], bins=30, duplicates='drop'))['vol_shares'].sum()
                global_vpoc_price = np.nan
                if not vp_global.empty:
    ...
    def _calculate_retail_sentiment_metrics(context: dict) -> dict:
        ...
        metrics = {
            ...
            'retail_fomo_volume_ratio': np.nan,
            'retail_panic_volume_ratio': np.nan,
            'retail_fomo_intensity': np.nan,
            'retail_panic_intensity': np.nan
        }
        atr = common_data['atr']
        ...
        window_size = max(10, min(100, len(hf_analysis_df_copy) // 100))
        ...
        hf_analysis_df_copy['price_acceleration'] = hf_analysis_df_copy['price_change_pct'].diff()
        ...
        prices_arr = hf_analysis_df_copy['price'].to_numpy(dtype=np.float64)
        volumes_arr = hf_analysis_df_copy['volume'].to_numpy(dtype=np.float64)
        amounts_arr = hf_analysis_df_copy['amount'].to_numpy(dtype=np.float64)
        types_arr = hf_analysis_df_copy['type_numeric'].to_numpy(dtype=np.int8)
        sell_price1s_arr = hf_analysis_df_copy['sell_price1'].to_numpy(dtype=np.float64)
        buy_price1s_arr = hf_analysis_df_copy['buy_price1'].to_numpy(dtype=np.float64)
        is_new_highs_arr = hf_analysis_df_copy['is_true_new_high'].to_numpy(dtype=bool)
        is_new_lows_arr = hf_analysis_df_copy['is_true_new_low'].to_numpy(dtype=bool)
        is_retail_arr = hf_analysis_df_copy['is_retail_trade'].to_numpy(dtype=bool)
        aggressive_buy_arr = hf_analysis_df_copy['aggressive_buy'].to_numpy(dtype=bool) if 'aggressive_buy' in hf_analysis_df_copy.columns else np.zeros_like(prices_arr, dtype=bool)
        aggressive_sell_arr = hf_analysis_df_copy['aggressive_sell'].to_numpy(dtype=bool) if 'aggressive_sell' in hf_analysis_df_copy.columns else np.zeros_like(prices_arr, dtype=bool)
        price_acceleration_arr = hf_analysis_df_copy['price_acceleration'].to_numpy(dtype=np.float64) if 'price_acceleration' in hf_analysis_df_copy.columns else np.zeros_like(prices_arr, dtype=np.float64)
        volume_zscore_arr = hf_analysis_df_copy['volume_zscore'].to_numpy(dtype=np.float64) if 'volume_zscore' in hf_analysis_df_copy.columns else np.zeros_like(prices_arr, dtype=np.float64)
        ...
        if total_fomo_volume > 0 and total_fomo_amount > 0:
            ...
            fomo_intensity_factor = 1.0 + min(2.0, fomo_volume_ratio * 5)
