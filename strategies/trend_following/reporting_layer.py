    ...
    async def prepare_db_records(self, stock_code: str, result_df: pd.DataFrame, score_details_df: pd.DataFrame, risk_details_df: pd.DataFrame, params: dict, result_timeframe: str) -> Tuple[List, List, List, List, List]:
        ...
        for trade_time, row in result_df.iterrows():
            ...
            daily_score_obj = StrategyDailyScore(
                ...
                strategy_name=strategy_name,
                offensive_score=int(row.get('entry_score', 0) if pd.notna(row.get('entry_score', 0)) else 0),
                risk_score=int(row.get('risk_score', 0) if pd.notna(row.get('risk_score', 0)) else 0),
                final_score=row.get('final_score', 0.0) if pd.notna(row.get('final_score', 0.0)) else 0.0,
                positional_score=int(setup_score if pd.notna(setup_score) else 0),
                dynamic_score=int(trigger_score if pd.notna(trigger_score) else 0),
                composite_score=int(playbook_synergy_score if pd.notna(playbook_synergy_score) else 0),
            ...
            if not score_details_df.empty and trade_time in score_details_df.index:
                ...
                for signal_name, score_value in active_offensive.items():
                    if signal_name in summary_score_names:
                        continue
