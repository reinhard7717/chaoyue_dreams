...
class FundFlowIntelligence:
    ...
    def diagnose_fund_flow_states(self, df: pd.DataFrame) -> Dict[str, pd.Series]:
        """
        【V1.0 新增】资金流原子信号诊断模块
        - 核心职责: 从三种资金流数据源中，诊断出各自的原子资金流信号。
        - 收益: 为后续的交叉验证和评分提供标准化的资金流情报。
        """
        print("        -> [资金流情报模块 V1.0] 启动，正在诊断资金流原子信号...")
        states = {}
        ...
        # 字段: net_mf_amount_D (净流入额)
        net_mf_amount_col = 'net_mf_amount_D'
        ...
        # 字段: net_amount_D (净流入额), buy_lg_amount_rate_D (大单买入占比)
        net_amount_ths_col = 'net_amount_D' # 注意：同花顺资金流的净额字段名可能与Tushare原始资金流重名，需要确保数据层处理好后缀
        buy_lg_amount_rate_ths_col = 'buy_lg_amount_rate_D'
        
        # 假设 IndicatorService 已经处理了列名冲突，例如加上了源后缀
        # 如果没有，这里需要根据实际的列名进行调整
        # 例如，如果同花顺的净额列名是 'net_amount_ths_D'
        
        # 检查是否存在同花顺资金流的净额列
        if 'net_amount_fund_flow_ths_D' in df.columns: # 假设 IndicatorService 已经添加了 'fund_flow_ths' 后缀
            ...
            print(f"          -> [警告] 缺少同花顺资金流列 'net_amount_fund_flow_ths_D'，跳过。")

        # 检查是否存在同花顺资金流的大单买入占比列
        if 'buy_lg_amount_rate_fund_flow_ths_D' in df.columns:
            ...
            print(f"          -> [警告] 缺少同花顺大单买入占比列 'buy_lg_amount_rate_fund_flow_ths_D'，跳过。")

        # --- 3. 东方财富资金流 (moneyflow_dc) ---
        # 字段: net_amount_D (净流入额), buy_elg_amount_rate_D (超大单买入占比)
        net_amount_dc_col = 'net_amount_D' # 同样，假设已处理重名
        buy_elg_amount_rate_dc_col = 'buy_elg_amount_rate_D'

        if 'net_amount_fund_flow_dc_D' in df.columns: # 假设 IndicatorService 已经添加了 'fund_flow_dc' 后缀
            ...
            print(f"          -> [警告] 缺少东方财富资金流列 'net_amount_fund_flow_dc_D'，跳过。")

        if 'buy_elg_amount_rate_fund_flow_dc_D' in df.columns:
            ...
            print(f"          -> [警告] 缺少东方财富超大单买入占比列 'buy_elg_amount_rate_fund_flow_dc_D'，跳过。")

        print("        -> [资金流情报模块 V1.0] 诊断完毕。")
        return states
