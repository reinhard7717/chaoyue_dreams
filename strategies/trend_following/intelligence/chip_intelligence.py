...
class ChipIntelligence:
    ...
    def _diagnose_axiom_trend_momentum(self, df: pd.DataFrame, periods: list) -> pd.Series:
        ...
        """
        required_signals = [
            'SCORE_CHIP_AXIOM_CONCENTRATION', 'SCORE_CHIP_AXIOM_COST_STRUCTURE', 'SCORE_CHIP_AXIOM_HOLDER_SENTIMENT',
        ...
        # 融合前三个公理作为整体健康度
        health_score = (
            (df['SCORE_CHIP_AXIOM_CONCENTRATION'].add(1)/2) *
            (df['SCORE_CHIP_AXIOM_COST_STRUCTURE'].add(1)/2) *
    ...
    def _diagnose_absorption_echo(self, df: pd.DataFrame, divergence_scores: pd.Series) -> pd.Series:
        ...
        """
        required_signals = ['SLOPE_5_close_D', 'main_force_net_inflow_rate_D']
        ...
        # 3. 主力回声 (主力逆势流入)
        mf_inflow = self._get_safe_series(df, df, 'main_force_net_inflow_rate_D', 0.0, method_name="_diagnose_absorption_echo")
    ...
    def _diagnose_distribution_whisper(self, df: pd.DataFrame, divergence_scores: pd.Series) -> pd.Series:
        ...
        # 3. 主力抽离 (主力顺势流出)
        mf_inflow = self._get_safe_series(df, df, 'main_force_net_inflow_rate_D', 0.0, method_name="_diagnose_distribution_whisper")
