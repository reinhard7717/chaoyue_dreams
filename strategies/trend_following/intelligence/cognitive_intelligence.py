...
class CognitiveIntelligence:
    ...
    def _deduce_suppressive_accumulation(self, df: pd.DataFrame, priors: Dict[str, pd.Series]) -> Dict[str, pd.Series]:
        """
        【V11.0 · 诡道吸筹深度感知版】贝叶斯推演：“主力打压吸筹”剧本
        - 核心升级:
            1. 精细化流动性对打压证据的调制：在流动性差时，放大打压证据权重。
            2. 引入“隐藏力量”证据：`SCORE_CHIP_COHERENT_DRIVE` 和 `SCORE_FF_AXIOM_CAPITAL_SIGNATURE` (正向部分)。
            3. 强化“欺骗”对打压证据的放大作用：当欺骗指数高时，额外放大打压证据权重。
            4. 引入“结构支撑”证据：`SCORE_STRUCT_AXIOM_STABILITY`。
            5. 优化趋势质量惩罚机制：引入非线性惩罚函数。
            6. 动态 `power_factor`：根据 `volatility_instability` 动态调整非线性放大因子。
            7. 增强“压制”证据的情境放大：在熊市/弱势趋势中，额外放大压制相关证据的权重。
            8. 引入“协同吸筹”证据：通过几何平均融合多个核心吸筹信号，捕捉协同效应。
            9. 调整证据权重：根据新增信号的直接性和重要性，重新分配权重。
        """
        ...
        base_weights_dict = {
            ...
            # V11.0 新增证据权重
            'chip_coherent_drive': 0.04, # 筹码同调驱动力 (隐藏力量) # 新增行
            'ff_capital_signature_patient': 0.03, # 资金流耐心资本 (隐藏力量) # 新增行
            'struct_stability': 0.03 # 结构稳定性 (结构支撑) # 新增行
        }
        ...
        # V11.0 新增证据获取
        raw_chip_coherent_drive = self._get_atomic_score(df, 'SCORE_CHIP_COHERENT_DRIVE', 0.0) # 新增行
        chip_coherent_drive_evidence = self._forge_dynamic_evidence(df, raw_chip_coherent_drive.clip(lower=0)) # 新增行

        raw_ff_capital_signature = self._get_atomic_score(df, 'SCORE_FF_AXIOM_CAPITAL_SIGNATURE', 0.0) # 新增行
        ff_capital_signature_patient_evidence = self._forge_dynamic_evidence(df, raw_ff_capital_signature.clip(lower=0)) # 新增行

        raw_struct_stability = self._get_atomic_score(df, 'SCORE_STRUCT_AXIOM_STABILITY', 0.0) # 新增行
        struct_stability_evidence = self._forge_dynamic_evidence(df, raw_struct_stability.clip(lower=0)) # 新增行
        ...
        synergistic_accumulation_components = [
            ...
            chip_coherent_drive_evidence, # 新增行: 添加新的协同吸筹组件
            ff_capital_signature_patient_evidence # 新增行: 添加新的协同吸筹组件
        ]
        ...
        if probe_date_for_loop is not None and probe_date_for_loop in df.index:
            ...
            print(f"       - 微观意图背离(原始): {raw_micro_axiom_divergence.loc[probe_date_for_loop]:.4f}, 负向证据: {micro_axiom_divergence_negative_evidence.loc[probe_date_for_loop]:.4f}")
            print(f"       - 吸筹拐点信号(原始): {raw_fusion_accumulation_inflection_point.loc[probe_date_for_loop]:.4f}, 证据: {fusion_accumulation_inflection_point_evidence.loc[probe_date_for_loop]:.4f}")
            print(f"       - 筹码同调驱动力(原始): {raw_chip_coherent_drive.loc[probe_date_for_loop]:.4f}, 证据: {chip_coherent_drive_evidence.loc[probe_date_for_loop]:.4f}") # 新增行
            print(f"       - 资金流耐心资本(原始): {raw_ff_capital_signature.loc[probe_date_for_loop]:.4f}, 证据: {ff_capital_signature_patient_evidence.loc[probe_date_for_loop]:.4f}") # 新增行
            print(f"       - 结构稳定性(原始): {raw_struct_stability.loc[probe_date_for_loop]:.4f}, 证据: {struct_stability_evidence.loc[probe_date_for_loop]:.4f}") # 新增行
        ...
        liquidity_mod = raw_liquidity_dynamics.clip(upper=0).abs() * 0.1 # 修改行: 仅在流动性为负时放大
        ...
        # V11.0 欺骗放大因子
        deception_amplification = raw_deception_index.clip(lower=0) * 0.1 # 新增行: 欺骗指数越高，放大作用越强
        ...
        adaptive_weights_per_date['price_falling'] += market_regime_mod + trend_quality_mod + volatility_mod + suppression_context_amplification + liquidity_mod + deception_amplification # 修改行: 添加欺骗放大
        adaptive_weights_per_date['deception'] += market_regime_mod + trend_quality_mod + sentiment_mod + suppression_context_amplification + liquidity_mod + deception_amplification # 修改行: 添加欺骗放大
        adaptive_weights_per_date['volume_atrophy'] += market_regime_mod + trend_quality_mod + sentiment_mod + suppression_context_amplification + liquidity_mod + deception_amplification # 修改行: 添加欺骗放大
        adaptive_weights_per_date['suppressive_accum_intensity'] += market_regime_mod + trend_quality_mod + volatility_mod + suppression_context_amplification + liquidity_mod + deception_amplification # 修改行: 添加欺骗放大
        adaptive_weights_per_date['suppressive_accum_intensity_slope'] += market_regime_mod + trend_quality_mod + volatility_mod + suppression_context_amplification + liquidity_mod + deception_amplification # 修改行: 添加欺骗放大
        adaptive_weights_per_date['shakeout_confirmation'] += market_regime_mod + trend_quality_mod + volatility_mod + suppression_context_amplification + liquidity_mod + deception_amplification # 修改行: 添加欺骗放大
        adaptive_weights_per_date['deceptive_accum'] += market_regime_mod + trend_quality_mod + sentiment_mod + suppression_context_amplification + liquidity_mod + deception_amplification # 修改行: 添加欺骗放大
        adaptive_weights_per_date['panic_washout_accum'] += market_regime_mod + trend_quality_mod + sentiment_mod + suppression_context_amplification + liquidity_mod + deception_amplification # 修改行: 添加欺骗放大
        adaptive_weights_per_date['price_downward_momentum'] += market_regime_mod + trend_quality_mod + volatility_mod + suppression_context_amplification + liquidity_mod + deception_amplification # 修改行: 添加欺骗放大
        adaptive_weights_per_date['pct_change_slope'] += market_regime_mod + trend_quality_mod + volatility_mod + suppression_context_amplification + liquidity_mod + deception_amplification # 修改行: 添加欺骗放大
        adaptive_weights_per_date['downward_resistance'] += market_regime_mod + trend_quality_mod + volatility_mod + suppression_context_amplification + liquidity_mod + deception_amplification # 修改行: 添加欺骗放大
        adaptive_weights_per_date['intraday_vwap_battlefield_negative'] += market_regime_mod + trend_quality_mod + volatility_mod + suppression_context_amplification + liquidity_mod + deception_amplification # 修改行: 添加欺骗放大
        adaptive_weights_per_date['micro_axiom_divergence_negative'] += market_regime_mod + trend_quality_mod + sentiment_mod + suppression_context_amplification + liquidity_mod + deception_amplification # 修改行: 添加欺骗放大
        ...
        adaptive_weights_per_date['fusion_accumulation_inflection_point'] += market_regime_mod + sentiment_mod + trend_quality_mod
        adaptive_weights_per_date['chip_coherent_drive'] += market_regime_mod + sentiment_mod + trend_quality_mod # 新增行
        adaptive_weights_per_date['ff_capital_signature_patient'] += market_regime_mod + sentiment_mod # 新增行
        adaptive_weights_per_date['struct_stability'] += market_regime_mod + sentiment_mod + trend_quality_mod # 新增行
        ...
        if probe_date_for_loop is not None and probe_date_for_loop in df.index:
            print(f"       - 市场状态调制: {market_regime_mod.loc[probe_date_for_loop]:.4f}, 趋势质量调制: {trend_quality_mod.loc[probe_date_for_loop]:.4f}, 情绪调制: {sentiment_mod.loc[probe_date_for_loop]:.4f}, 流动性调制: {liquidity_mod.loc[probe_date_for_loop]:.4f}, 压制情境放大: {suppression_context_amplification.loc[probe_date_for_loop]:.4f}, 欺骗放大: {deception_amplification.loc[probe_date_for_loop]:.4f}") # 修改行: 打印新的调制因子
            # 打印特定日期的自适应权重
        ...
        evidence_list = [
            ...
            fusion_accumulation_inflection_point_evidence,
            # V11.0 新增证据
            chip_coherent_drive_evidence, # 新增行
            ff_capital_signature_patient_evidence, # 新增行
            struct_stability_evidence # 新增行
        ]
        ...
        bullish_trend_penalty_factor = (trend_quality_score.clip(0.6, 1.0) - 0.6).pow(2) * 2.5 # 修改行: 非线性惩罚，0.6到1.0，惩罚因子从0到1.0
        ...
        if probe_date_for_loop is not None and probe_date_for_loop in df.index:
            ...
            print(f"       - 趋势质量惩罚因子: {bullish_trend_penalty_factor.loc[probe_date_for_loop]:.4f}")
