...
class CognitiveIntelligence:
    ...
    def _deduce_suppressive_accumulation(self, df: pd.DataFrame, priors: Dict[str, pd.Series]) -> Dict[str, pd.Series]:
        """
        【V3.3 · 数据帧上下文修复版】贝叶斯推演：“主力打压吸筹”剧本
        - 核心升级: 不再直接使用原始证据，而是先通过 `_forge_dynamic_evidence` 进行动态锻造。
        - 引入市场矛盾（看涨背离）作为证据。
        - 【V3.3 修复】接收并使用 df 参数，确保索引上下文统一。
        """
    ...
    def _deduce_distribution_at_high(self, df: pd.DataFrame, priors: Dict[str, pd.Series]) -> Dict[str, pd.Series]:
        ...
        main_force_holding_inverse = self._forge_dynamic_evidence(df, df, 1 - main_force_holding_strength)
        capital_confrontation_bearish = self._forge_dynamic_evidence(df, df, self._get_fused_score(df, 'FUSION_BIPOLAR_CAPITAL_CONFRONTATION', 0.0).clip(upper=0).abs())
        price_overextension_risk = self._forge_dynamic_evidence(df, df, self._get_fused_score(df, 'FUSION_BIPOLAR_PRICE_OVEREXTENSION_INTENT', 0.0).clip(upper=0).abs())
        low_upward_efficiency = self._forge_dynamic_evidence(df, df, (1 - self._get_atomic_score(df, 'SCORE_BEHAVIOR_UPWARD_EFFICIENCY', 0.5)).clip(0, 1))
        profit_vs_flow_bearish = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'PROCESS_META_PROFIT_VS_FLOW', 0.0).clip(upper=0).abs())
        chip_dispersion_evidence = self._forge_dynamic_evidence(df, df, (1 - self._get_atomic_score(df, 'SCORE_CHIP_AXIOM_CONCENTRATION', 0.5)).clip(0, 1))
        market_contradiction_bearish = self._forge_dynamic_evidence(df, df, self._get_fused_score(df, 'FUSION_BIPOLAR_MARKET_CONTRADICTION', 0.0).clip(upper=0).abs())
        upper_shadow_pressure = self._forge_dynamic_evidence(df, df, self._get_fused_score(df, 'FUSION_BIPOLAR_UPPER_SHADOW_INTENT', 0.0).clip(upper=0).abs())
        fund_flow_bearish_divergence = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'SCORE_FUND_FLOW_BEARISH_DIVERGENCE', 0.0))
        chip_bearish_divergence = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'SCORE_CHIP_BEARISH_DIVERGENCE', 0.0))
        dip_absorption_power = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'dip_absorption_power_D', 0.0))
    ...
    def _deduce_trend_exhaustion_risk(self, df: pd.DataFrame, priors: Dict[str, pd.Series]) -> Dict[str, pd.Series]:
        ...
        main_force_holding_strength = self._get_main_force_holding_strength(df)
        main_force_holding_inverse = self._forge_dynamic_evidence(df, df, 1 - main_force_holding_strength)
        price_momentum_divergence = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'PROCESS_META_PRICE_VS_MOMENTUM_DIVERGENCE', 0.0).clip(lower=0))
        stagnation_evidence = self._forge_dynamic_evidence(df, df, (1 - self._get_atomic_score(df, 'SCORE_BEHAVIOR_UPWARD_EFFICIENCY', 0.5)).clip(0, 1))
        raw_price_overextension_score = self._get_fused_score(df, 'FUSION_BIPOLAR_PRICE_OVEREXTENSION_INTENT', 0.0)
        price_overextension_risk = self._forge_dynamic_evidence(df, df, raw_price_overextension_score.clip(upper=0).abs())
        winner_conviction_decay = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'PROCESS_META_WINNER_CONVICTION_DECAY', 0.0))
        raw_capital_confrontation_score = self._get_fused_score(df, 'FUSION_BIPOLAR_CAPITAL_CONFRONTATION', 0.0)
        capital_retreat_evidence = self._forge_dynamic_evidence(df, df, raw_capital_confrontation_score.clip(upper=0).abs())
        fund_flow_bearish_divergence = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'SCORE_FUND_FLOW_BEARISH_DIVERGENCE', 0.0))
        retail_fomo_retreat_risk = self._forge_dynamic_evidence(df, df, self._get_playbook_score(df, 'COGNITIVE_RISK_RETAIL_FOMO_RETREAT', 0.0), is_probability=True)
        chip_dispersion_evidence = self._forge_dynamic_evidence(df, df, (1 - self._get_atomic_score(df, 'SCORE_CHIP_AXIOM_CONCENTRATION', 0.5)).clip(0, 1))
        chip_bearish_divergence = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'SCORE_CHIP_BEARISH_DIVERGENCE', 0.0))
        long_term_profit_distribution_risk = self._forge_dynamic_evidence(df, df, self._get_playbook_score(df, 'COGNITIVE_RISK_LONG_TERM_PROFIT_DISTRIBUTION', 0.0), is_probability=True)
        raw_structural_trend_form_score = self._get_atomic_score(df, 'SCORE_STRUCT_AXIOM_TREND_FORM', 0.0)
        structural_deterioration = self._forge_dynamic_evidence(df, df, raw_structural_trend_form_score.clip(upper=0).abs())
        raw_market_contradiction_score = self._get_fused_score(df, 'FUSION_BIPOLAR_MARKET_CONTRADICTION', 0.0)
        market_contradiction_bearish = self._forge_dynamic_evidence(df, df, raw_market_contradiction_score.clip(upper=0).abs())
        raw_upper_shadow_intent_score = self._get_fused_score(df, 'FUSION_BIPOLAR_UPPER_SHADOW_INTENT', 0.0)
        upper_shadow_pressure_risk = self._forge_dynamic_evidence(df, df, raw_upper_shadow_intent_score.clip(upper=0).abs())
        cyclical_top_risk = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'COGNITIVE_RISK_CYCLICAL_TOP', 0.0))
        trend_quality_inverse = self._forge_dynamic_evidence(df, df, 1 - self._get_fused_score(df, 'FUSION_BIPOLAR_TREND_QUALITY', 0.0).clip(lower=0))
        new_high_strength_inverse = self._forge_dynamic_evidence(df, df, 1 - self._get_atomic_score(df, 'CONTEXT_NEW_HIGH_STRENGTH', 0.0))
        dip_absorption_power = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'dip_absorption_power_D', 0.0))
        dip_absorption_inverse = (1 - dip_absorption_power).clip(0, 1)
        evidence_scores = np.stack([
    ...
    def _deduce_chasing_accumulation(self, df: pd.DataFrame, priors: Dict[str, pd.Series]) -> Dict[str, pd.Series]:
        ...
        capital_confrontation = self._forge_dynamic_evidence(df, df, self._get_fused_score(df, 'FUSION_BIPOLAR_CAPITAL_CONFRONTATION', 0.0).clip(lower=0))
        price_change_bipolar = normalize_to_bipolar(self._get_atomic_score(df, 'pct_change_D'), df.index, 21)
        price_rising_evidence = self._forge_dynamic_evidence(df, df, price_change_bipolar.clip(lower=0))
        efficiency_evidence = self._forge_dynamic_evidence(df, df, normalize_score(self._get_atomic_score(df, 'VPA_EFFICIENCY_D'), df.index, 55))
        rally_intent_evidence = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'PROCESS_META_MAIN_FORCE_RALLY_INTENT', 0.0).clip(lower=0))
        conviction_evidence = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'PROCESS_META_WINNER_CONVICTION', 0.0).clip(lower=0))
        process_evidence = (rally_intent_evidence * conviction_evidence).pow(0.5)
        chip_evidence = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'SCORE_CHIP_AXIOM_CONCENTRATION', 0.0).clip(lower=0))
        structural_consensus_evidence = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'SCORE_CHIP_STRUCTURAL_CONSENSUS', 0.0))
        pullback_confirmation_evidence = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'SCORE_PATTERN_PULLBACK_CONFIRMATION', 0.0))
        duofangpao_evidence = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'SCORE_PATTERN_DUOFANGPAO', 0.0))
        evidence_scores = np.stack([
    ...
    def _deduce_capitulation_reversal(self, df: pd.DataFrame, priors: Dict[str, pd.Series]) -> Dict[str, pd.Series]:
        ...
        upward_pressure = self._forge_dynamic_evidence(df, df, self._get_fused_score(df, 'FUSION_BIPOLAR_MARKET_PRESSURE', 0.0).clip(lower=0))
        price_rebound_evidence = self._forge_dynamic_evidence(df, df, normalize_score(self._get_atomic_score(df, 'dip_absorption_power_D'), df.index, 55))
        ...
        volume_evidence = self._forge_dynamic_evidence(df, df, normalize_score(volume_spike, df.index, 55))
        process_evidence = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'PROCESS_META_LOSER_CAPITULATION', 0.0).clip(lower=0))
        micro_evidence = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'PROCESS_META_MICRO_BEHAVIOR_BOTTOM_REVERSAL', 0.0).clip(lower=0))
        ww1_mode = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'IS_WW1_D', 0.0).astype(float))
        evidence_scores = np.stack([
    ...
    def _deduce_leading_dragon_awakening(self, df: pd.DataFrame, priors: Dict[str, pd.Series]) -> Dict[str, pd.Series]:
        ...
        capital_confrontation = self._forge_dynamic_evidence(df, df, self._get_fused_score(df, 'FUSION_BIPOLAR_CAPITAL_CONFRONTATION', 0.0).clip(lower=0))
        breakout_quality = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'breakout_quality_score_D', 0.0))
        sector_sync = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'PROCESS_META_STOCK_SECTOR_SYNC', 0.0).clip(lower=0))
        relative_strength = self._forge_dynamic_evidence(df, df, normalize_score(self._get_atomic_score(df, 'industry_strength_rank_D', 0.5), df.index, 55))
        bazhan_mode = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'IS_BAZHAN_D', 0.0).astype(float))
        structural_consensus_evidence = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'SCORE_CHIP_STRUCTURAL_CONSENSUS', 0.0))
        evidence_scores = np.stack([
    ...
    def _deduce_divergence_reversal(self, df: pd.DataFrame, priors: Dict[str, pd.Series]) -> Dict[str, pd.Series]:
        ...
        df_index = df.index # [代码修改] 使用 df.index
        price_momentum_divergence = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'PROCESS_META_PRICE_VS_MOMENTUM_DIVERGENCE', 0.0))
        fund_flow_bearish_divergence = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'SCORE_FUND_FLOW_BEARISH_DIVERGENCE', 0.0))
        chip_bearish_divergence = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'SCORE_CHIP_BEARISH_DIVERGENCE', 0.0))
        trend_exhaustion_risk = self._forge_dynamic_evidence(df, df, self._get_playbook_score(df, 'COGNITIVE_RISK_TREND_EXHAUSTION', 0.0), is_probability=True)
        raw_market_contradiction_score = self._get_fused_score(df, 'FUSION_BIPOLAR_MARKET_CONTRADICTION', 0.0)
        market_contradiction_bearish = self._forge_dynamic_evidence(df, df, raw_market_contradiction_score.clip(upper=0).abs())
        winner_conviction_decay = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'PROCESS_META_WINNER_CONVICTION_DECAY', 0.0))
        evidence_scores = np.stack([
    ...
    def _deduce_sector_rotation_vanguard(self, df: pd.DataFrame, priors: Dict[str, pd.Series]) -> Dict[str, pd.Series]:
        ...
        sector_flow = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'PROCESS_META_FUND_FLOW_BOTTOM_REVERSAL', 0.0).clip(lower=0))
        price_position = self._forge_dynamic_evidence(df, df, 1 - normalize_score(self._get_atomic_score(df, 'BIAS_144_D', 0.0), df.index, 144))
        chip_cleanliness = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'SCORE_CHIP_CLEANLINESS', 0.0))
        hot_sector_cooling = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'PROCESS_META_HOT_SECTOR_COOLING', 0.0))
    ...
    def _deduce_energy_compression_breakout(self, df: pd.DataFrame, priors: Dict[str, pd.Series]) -> Dict[str, pd.Series]:
        ...
        volatility_compression = self._forge_dynamic_evidence(df, df, 1 - normalize_score(bbw, df_index, 144))
        volume_atrophy_raw_score = self._get_atomic_score(df, 'SCORE_BEHAVIOR_VOLUME_ATROPHY', 0.0)
        volume_atrophy = self._forge_dynamic_evidence(df, df, volume_atrophy_raw_score)
        ...
        if entropy_col:
            ...
            orderliness_score = self._forge_dynamic_evidence(df, df, 1 - normalize_score(entropy, df_index, 144))
        else:
            orderliness_score = pd.Series(0.5, index=df_index)
        pct_change_raw = self._get_atomic_score(df, 'pct_change_D', 0.0)
        price_burst_evidence = self._forge_dynamic_evidence(df, df, pct_change_raw.clip(lower=0), is_probability=False)
        volume_burst_raw = self._get_atomic_score(df, 'SCORE_BEHAVIOR_VOLUME_BURST', 0.0)
        volume_burst_evidence = self._forge_dynamic_evidence(df, df, volume_burst_raw, is_probability=False)
    ...
    def _deduce_long_term_profit_distribution_risk(self, df: pd.DataFrame, priors: Dict[str, pd.Series]) -> Dict[str, pd.Series]:
        ...
        main_force_holding_strength = self._get_main_force_holding_strength(df)
        main_force_holding_inverse = self._forge_dynamic_evidence(df, df, 1 - main_force_holding_strength)
        long_term_profit_decay = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'PROCESS_META_WINNER_CONVICTION_DECAY', 0.0))
        chip_dispersion = self._forge_dynamic_evidence(df, df, (1 - self._get_atomic_score(df, 'SCORE_CHIP_AXIOM_CONCENTRATION', 0.5)).clip(0, 1))
        capital_outflow = self._forge_dynamic_evidence(df, df, self._get_fused_score(df, 'FUSION_BIPOLAR_CAPITAL_CONFRONTATION', 0.0).clip(upper=0).abs())
        dip_absorption_power = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'dip_absorption_power_D', 0.0))
        dip_absorption_inverse = (1 - dip_absorption_power).clip(0, 1)
        evidence_scores = np.stack([
    ...
    def _deduce_market_uncertainty_risk(self, df: pd.DataFrame, priors: Dict[str, pd.Series]) -> Dict[str, pd.Series]:
        ...
        regime_neutrality = self._forge_dynamic_evidence(df, df, 1 - self._get_fused_score(df, 'FUSION_BIPOLAR_MARKET_REGIME', 0.0).abs())
        low_trend_quality = self._forge_dynamic_evidence(df, df, 1 - self._get_fused_score(df, 'FUSION_BIPOLAR_TREND_QUALITY', 0.0).abs())
        entropy_col = next((col for col in df.columns if col.startswith('SAMPLE_ENTROPY_')), None)
        if entropy_col:
            high_entropy = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, entropy_col, 0.5))
        else:
    ...
    def _deduce_retail_fomo_retreat_risk(self, df: pd.DataFrame, priors: Dict[str, pd.Series]) -> Dict[str, pd.Series]:
        ...
        retail_inflow = self._forge_dynamic_evidence(df, df, raw_retail_inflow_score.clip(lower=0))
        raw_mf_confrontation_score = self._get_fused_score(df, 'FUSION_BIPOLAR_CAPITAL_CONFRONTATION', 0.0)
        main_force_outflow = self._forge_dynamic_evidence(df, df, raw_mf_confrontation_score.clip(upper=0).abs())
        raw_price_rising_score = normalize_to_bipolar(self._get_atomic_score(df, 'pct_change_D'), df.index, 21)
        price_rising = self._forge_dynamic_evidence(df, df, raw_price_rising_score.clip(lower=0))
        chip_dispersion = self._forge_dynamic_evidence(df, df, (1 - self._get_atomic_score(df, 'SCORE_CHIP_AXIOM_CONCENTRATION', 0.5)).clip(0, 1))
        dip_absorption_power = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'dip_absorption_power_D', 0.0))
        dip_absorption_inverse = (1 - dip_absorption_power).clip(0, 1)
        evidence_scores = np.stack([
    ...
    def _deduce_harvest_confirmation_risk(self, df: pd.DataFrame, priors: Dict[str, pd.Series]) -> Dict[str, pd.Series]:
        ...
        main_force_holding_strength = self._get_main_force_holding_strength(df)
        main_force_holding_inverse = self._forge_dynamic_evidence(df, df, 1 - main_force_holding_strength)
        high_distribution_risk = self._forge_dynamic_evidence(df, df, self._get_playbook_score(df, 'COGNITIVE_RISK_DISTRIBUTION_AT_HIGH', 0.0), is_probability=True)
        high_t0_efficiency = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'PROCESS_META_PROFIT_VS_FLOW', 0.0).clip(upper=0).abs())
        winner_conviction_decay = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'PROCESS_META_WINNER_CONVICTION_DECAY', 0.0))
        dip_absorption_power = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'dip_absorption_power_D', 0.0))
        dip_absorption_inverse = (1 - dip_absorption_power).clip(0, 1)
        evidence_scores = np.stack([
    ...
    def _deduce_bull_trap_distribution_risk(self, df: pd.DataFrame, priors: Dict[str, pd.Series]) -> Dict[str, pd.Series]:
        ...
        main_force_holding_strength = self._get_main_force_holding_strength(df)
        main_force_holding_inverse = self._forge_dynamic_evidence(df, df, 1 - main_force_holding_strength)
        price_rising = self._forge_dynamic_evidence(df, df, normalize_to_bipolar(self._get_atomic_score(df, 'pct_change_D'), df.index, 21).clip(lower=0))
        raw_micro_deception_score = self._get_atomic_score(df, 'SCORE_MICRO_AXIOM_DECEPTION', 0.0)
        micro_deception_bearish = self._forge_dynamic_evidence(df, df, raw_micro_deception_score.clip(upper=0).abs())
        raw_upper_shadow_intent_score = self._get_fused_score(df, 'FUSION_BIPOLAR_UPPER_SHADOW_INTENT', 0.0)
        upper_shadow_pressure = self._forge_dynamic_evidence(df, df, raw_upper_shadow_intent_score.clip(upper=0).abs())
        chip_dispersion = self._forge_dynamic_evidence(df, df, (1 - self._get_atomic_score(df, 'SCORE_CHIP_AXIOM_CONCENTRATION', 0.5)).clip(0, 1))
        raw_mf_confrontation_score = self._get_fused_score(df, 'FUSION_BIPOLAR_CAPITAL_CONFRONTATION', 0.0)
        main_force_outflow = self._forge_dynamic_evidence(df, df, raw_mf_confrontation_score.clip(upper=0).abs())
        raw_profit_vs_flow_score = self._get_atomic_score(df, 'PROCESS_META_PROFIT_VS_FLOW', 0.0)
        profit_vs_flow_bearish = self._forge_dynamic_evidence(df, df, raw_profit_vs_flow_score.clip(upper=0).abs())
        winner_conviction_decay = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'PROCESS_META_WINNER_CONVICTION_DECAY', 0.0))
        retail_fomo_retreat_risk = self._forge_dynamic_evidence(df, df, self._get_playbook_score(df, 'COGNITIVE_RISK_RETAIL_FOMO_RETREAT', 0.0), is_probability=True)
        raw_price_overextension_score = self._get_fused_score(df, 'FUSION_BIPOLAR_PRICE_OVEREXTENSION_INTENT', 0.0)
        price_overextension_risk = self._forge_dynamic_evidence(df, df, raw_price_overextension_score.clip(upper=0).abs())
    ...
    def _deduce_liquidity_trap_risk(self, df: pd.DataFrame, priors: Dict[str, pd.Series]) -> Dict[str, pd.Series]:
        ...
        capital_outflow = self._forge_dynamic_evidence(df, df, self._get_fused_score(df, 'FUSION_BIPOLAR_CAPITAL_CONFRONTATION', 0.0).clip(upper=0).abs())
        volume_apathy = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'SCORE_BEHAVIOR_VOLUME_ATROPHY', 0.0))
        volatility_contraction = self._forge_dynamic_evidence(df, df, 1 - self._get_atomic_score(df, 'SCORE_FOUNDATION_AXIOM_VOLATILITY', 0.0).clip(lower=0))
        dip_absorption_power = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'dip_absorption_power_D', 0.0))
        dip_absorption_inverse = (1 - dip_absorption_power).clip(0, 1)
        evidence_scores = np.stack([
    ...
    def _deduce_t0_arbitrage_pressure_risk(self, df: pd.DataFrame, priors: Dict[str, pd.Series]) -> Dict[str, pd.Series]:
        ...
        high_t0_efficiency = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'PROCESS_META_PROFIT_VS_FLOW', 0.0).clip(upper=0).abs())
        capital_outflow = self._forge_dynamic_evidence(df, df, self._get_fused_score(df, 'FUSION_BIPOLAR_CAPITAL_CONFRONTATION', 0.0).clip(upper=0).abs())
        micro_deception_bearish = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'SCORE_MICRO_AXIOM_DECEPTION', 0.0).clip(upper=0).abs())
        dip_absorption_power = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'dip_absorption_power_D', 0.0))
        dip_absorption_inverse = (1 - dip_absorption_power).clip(0, 1)
        evidence_scores = np.stack([
    ...
    def _deduce_key_support_break_risk(self, df: pd.DataFrame, priors: Dict[str, pd.Series]) -> Dict[str, pd.Series]:
        ...
        downward_pressure = self._forge_dynamic_evidence(df, df, self._get_fused_score(df, 'FUSION_BIPOLAR_MARKET_PRESSURE', 0.0).clip(upper=0).abs())
        low_structural_stability = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'SCORE_STRUCT_AXIOM_STABILITY', 0.0).clip(upper=0).abs())
        weak_foundation_trend = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'SCORE_FOUNDATION_AXIOM_TREND', 0.0).clip(upper=0).abs())
        loser_capitulation = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'PROCESS_META_LOSER_CAPITULATION', 0.0))
        dip_absorption_power = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'dip_absorption_power_D', 0.0))
        dip_absorption_inverse = (1 - dip_absorption_power).clip(0, 1)
        ...
        price_above_ma_inverse = self._forge_dynamic_evidence(df, df, 1 - price_above_ma_score)
        evidence_scores = np.stack([
    ...
    def _deduce_high_level_structural_collapse_risk(self, df: pd.DataFrame, priors: Dict[str, pd.Series]) -> Dict[str, pd.Series]:
        ...
        main_force_holding_inverse = self._forge_dynamic_evidence(df, df, 1 - main_force_holding_strength)
        high_distribution_risk = self._forge_dynamic_evidence(df, df, self._get_playbook_score(df, 'COGNITIVE_RISK_DISTRIBUTION_AT_HIGH', 0.0), is_probability=True)
        structural_trend_deterioration = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'SCORE_STRUCT_AXIOM_TREND_FORM', 0.0).clip(upper=0).abs())
        retail_fomo_retreat = self._forge_dynamic_evidence(df, df, self._get_playbook_score(df, 'COGNITIVE_RISK_RETAIL_FOMO_RETREAT', 0.0), is_probability=True)
        chip_dispersion = self._forge_dynamic_evidence(df, df, (1 - self._get_atomic_score(df, 'SCORE_CHIP_AXIOM_CONCENTRATION', 0.5)).clip(0, 1))
        dip_absorption_power = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'dip_absorption_power_D', 0.0))
        dip_absorption_inverse = (1 - dip_absorption_power).clip(0, 1)
        evidence_scores = np.stack([
    ...
    def _deduce_stealth_bottoming_divergence(self, df: pd.DataFrame, priors: Dict[str, pd.Series]) -> Dict[str, pd.Series]:
        ...
        df_index = df.index # [代码修改] 使用 df.index
        downward_momentum_decay = self._forge_dynamic_evidence(df, df, 1 - self._get_atomic_score(df, 'SCORE_BEHAVIOR_PRICE_DOWNWARD_MOMENTUM', 0.0))
        price_accel_positive = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'ACCEL_5_close_D', 0.0).clip(lower=0))
        behavior_bottom_reversal = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'PROCESS_META_BEHAVIOR_BOTTOM_REVERSAL', 0.0))
        volume_atrophy_strong = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'SCORE_BEHAVIOR_VOLUME_ATROPHY', 0.0))
        fund_flow_consensus_positive = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'SCORE_FF_AXIOM_CONSENSUS', 0.0).clip(lower=0))
        power_transfer_positive = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'PROCESS_META_POWER_TRANSFER', 0.0).clip(lower=0))
        chip_concentration_positive = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'SCORE_CHIP_AXIOM_CONCENTRATION', 0.0).clip(lower=0))
        stealth_accumulation_process = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'PROCESS_META_STEALTH_ACCUMULATION', 0.0))
        cost_advantage_trend_positive = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'PROCESS_META_COST_ADVANTAGE_TREND', 0.0).clip(lower=0))
        loser_capitulation_process = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'PROCESS_META_LOSER_CAPITULATION', 0.0))
        structural_consensus_evidence = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'SCORE_CHIP_STRUCTURAL_CONSENSUS', 0.0))
        evidence_scores = np.stack([
    ...
    def _deduce_micro_absorption_divergence(self, df: pd.DataFrame, priors: Dict[str, pd.Series]) -> Dict[str, pd.Series]:
        ...
        df_index = df.index # [代码修改] 使用 df.index
        price_down_momentum_high = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'SCORE_BEHAVIOR_PRICE_DOWNWARD_MOMENTUM', 0.0))
        price_stabilization = self._forge_dynamic_evidence(df, df, 1 - self._get_atomic_score(df, 'pct_change_D', 0.0).abs())
        price_weak_or_stable_context = np.maximum(price_down_momentum_high, price_stabilization)
        volume_atrophy_context = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'SCORE_BEHAVIOR_VOLUME_ATROPHY', 0.0))
        counterparty_exhaustion = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'counterparty_exhaustion_index_D', 0.0))
        selling_pressure_decreasing = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'SLOPE_5_active_selling_pressure_D', 0.0).clip(upper=0).abs())
        dip_absorption_power = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'dip_absorption_power_D', 0.0))
        buying_support_increasing = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'SLOPE_5_active_buying_support_D', 0.0).clip(lower=0))
        structural_consensus_evidence = self._forge_dynamic_evidence(df, df, self._get_atomic_score(df, 'SCORE_CHIP_STRUCTURAL_CONSENSUS', 0.0))
        evidence_scores = np.stack([
