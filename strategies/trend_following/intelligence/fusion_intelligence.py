...
class FusionIntelligence:
    ...
    def _get_normalized_risk_score(self, df: pd.DataFrame, signal_name: str, norm_window: int, default_value: float = 0.0, mtf_norm_weights: Dict = None, debug_info: Optional[Tuple[bool, pd.Timestamp, str]] = None) -> pd.Series:
        """
        【V1.4 · NaN传播与MTF归一化版】获取并归一化风险信号。处理双极性信号的正负部分。
        - 【V1.3 变更】当原始信号为NaN时，归一化后也为NaN，以暴露问题。
        - 【V1.4 增强】增加详细调试打印，输出原始信号、处理后的信号以及归一化后的分数。
        """
        is_debug_enabled, probe_ts, method_name_caller = debug_info if debug_info else (False, None, "未知方法") # 更改变量名避免冲突
    ...
    def _synthesize_price_overextension_intent(self, df: pd.DataFrame, debug_info: Optional[Tuple[bool, pd.Timestamp, str]] = None) -> Dict[str, pd.Series]:
        """
        【V4.6 · 天道裁决版 - 依赖解耦与强化】冶炼“价格超买意图” (Price Overextension Intent)
        - 核心升华: 引入“趋势质量”作为天道背景，对“三位一体”的初审判决进行终极裁决。
        - 终章心法: 罪罚需与时势相符。逆势之罪，天道加诛。此法之后，再无增益。
        - 【V4.6 增强】将对融合信号 `FUSION_BIPOLAR_TREND_QUALITY` 的依赖替换为基础信号 `SCORE_STRUCT_AXIOM_TREND_FORM`，以解耦依赖。
        ...
        #     bearish_intent_total_weight = sum(bearish_intent_sources.values())
        #     print(f"        熊市意图总权重: {bearish_intent_total_weight:.4f}")
        for signal, weight in bearish_intent_sources.items():
            ...
            # if is_debug_enabled and probe_ts and probe_ts in df.index:
            #     print(f"        {signal}: 原始值={score.loc[probe_ts]:.4f}, 贡献={score.loc[probe_ts] * weight:.4f}")
        ...
        #     bullish_intent_total_weight = sum(bullish_intent_sources.values())
        #     print(f"        牛市意图总权重: {bullish_intent_total_weight:.4f}")
        ...
        #     bearish_pressure_total_weight = sum(config['weight'] for config in bearish_pressure_sources.values())
        #     print(f"        熊市压力总权重: {bearish_pressure_total_weight:.4f}")
        for signal_name, config in bearish_pressure_sources.items(): # 遍历新的字典结构
            weight = config['weight']
            part = config['part']
            score_raw = self._get_atomic_score(df, signal_name, 0.0, debug_info).fillna(0.0) # 获取原始信号
            score_processed = pd.Series(0.0, index=df_index, dtype=np.float32)
            if part == 'negative':
                score_processed = score_raw.clip(upper=0).abs()
            elif part == 'positive':
                score_processed = score_raw.clip(lower=0)
            else: # 默认情况，直接使用
                score_processed = score_raw
            bearish_pressure += score_processed * weight
            if is_debug_enabled and probe_ts and probe_ts in df.index:
                print(f"        {signal_name} ({part}): 原始值={score_raw.loc[probe_ts]:.4f}, 处理后值={score_processed.loc[probe_ts]:.4f}, 贡献={score_processed.loc[probe_ts] * weight:.4f}")
        ...
        #     bullish_pressure_total_weight = sum(bullish_pressure_sources.values())
        #     print(f"        牛市压力总权重: {bullish_pressure_total_weight:.4f}")
        ...
        # 2.4 终审裁决
        final_score_unclipped = (raw_bipolar_intent * contextual_modulator)
        final_score = final_score_unclipped.clip(-1, 1)
        # if is_debug_enabled and probe_ts and probe_ts in df.index:
        #     print(f"      [融合层调试] {method_name} @ {probe_ts.strftime('%Y-%m-%d')}: 终审裁决 (未裁剪): {final_score_unclipped.loc[probe_ts]:.4f}")
    ...
    def _synthesize_distribution_pressure(self, df: pd.DataFrame, debug_info: Optional[Tuple[bool, pd.Timestamp, str]] = None) -> Dict[str, pd.Series]:
        ...
        - 【V1.9 增强】将对融合信号的依赖替换为基础信号，以解耦依赖。
                      ...
                      **修正 `raw_panic_dampener` 部分获取 `panic_signal` 的方式，改为调用 `_get_normalized_risk_score`。**
        """
        method_name = "_synthesize_distribution_pressure"
        is_debug_enabled, probe_ts, _ = debug_info if debug_info else (False, None, method_name)
        # if is_debug_enabled and probe_ts and probe_ts in df.index:
        #     print(f"  -- [融合层调试] {method_name} @ {probe_ts.strftime('%Y-%m-%d')}: 正在冶炼“派发压力”...")
        #     print(f"      [融合层调试] {method_name} @ {probe_ts.strftime('%Y-%m-%d')}: debug_info: {debug_info}")
        #     print(f"      [融合层调试] {method_name} @ {probe_ts.strftime('%Y-%m-%d')}: self.params keys: {list(self.params.keys())}")
        states = {}
        df_index = df.index
        # 修正参数加载逻辑：直接从 self.params 获取
        ...
        # if is_debug_enabled and probe_ts and probe_ts in df.index:
        #     print(f"      [融合层调试] {method_name} @ {probe_ts.strftime('%Y-%m-%d')}: mfdi_signal_weights: {mfdi_signal_weights}")
        #     print(f"      [融合层调试] {method_name} @ {probe_ts.strftime('%Y-%m-%d')}: MFDI总权重: {mfdi_total_weight:.4f}")
        #     print(f"      [融合层调试] {probe_ts.strftime('%Y-%m-%d')}: --- MFDI组件原始信号 ---")
        ...
        # if is_debug_enabled and probe_ts and probe_ts in df.index:
        #     print(f"      [融合层调试] {method_name} @ {probe_ts.strftime('%Y-%m-%d')}: RAW总权重: {raw_total_weight:.4f}")
        #     print(f"      [融合层调试] {probe_ts.strftime('%Y-%m-%d')}: --- RAW组件原始信号 ---")
        ...
        if get_param_value(raw_panic_dampener_params.get('enabled'), False):
            ...
            # if is_debug_enabled and probe_ts and probe_ts in df.index:
            #     print(f"      [融合层调试] {method_name} @ {probe_ts.strftime('%Y-%m-%d')}: 恐慌抑制信号 '{panic_signal_name}' (归一化: {panic_signal.loc[probe_ts]:.4f})")
            #     print(f"      [融合层调试] {method_name} @ {probe_ts.strftime('%Y-%m-%d')}: 恐慌抑制器 (panic_dampener): {panic_dampener.loc[probe_ts]:.4f}")
        # if is_debug_enabled and probe_ts and probe_ts in df.index:
        ...
        # if is_debug_enabled and probe_ts and probe_ts in df.index:
        #     print(f"      [融合层调试] {method_name} @ {probe_ts.strftime('%Y-%m-%d')}: MSF总权重: {msf_total_weight:.4f}")
