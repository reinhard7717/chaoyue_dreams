...
class BehavioralIntelligence:
    ...
    def _diagnose_deception_index(self, df: pd.DataFrame, is_debug_enabled: bool, probe_ts: Optional[pd.Timestamp]) -> pd.Series:
        ...
        if is_debug_enabled and probe_ts and not df.empty and probe_ts == df.index[-1]:
            ...
            print(f"        - 相关欺骗强度 (Relevant Deception Intensity): {relevant_deception_intensity.loc[probe_ts]:.4f}")
            print(f"        - 欺骗证据分数: {deception_evidence_score.loc[probe_ts]:.4f}")
            print(f"        - 证据放大器: {evidence_amplifier.loc[probe_ts]:.4f}")
    ...
    def _diagnose_divergence_quality(self, df: pd.DataFrame, absorption_strength: pd.Series, distribution_intent: pd.Series, is_debug_enabled: bool, probe_ts: Optional[pd.Timestamp]) -> Tuple[pd.Series, pd.Series]:
        ...
        if is_debug_enabled and probe_ts and not df.empty and probe_ts == df.index[-1]:
            ...
            print(f"        - loser_pain_index_D: {signals_data['loser_pain_index_D'].loc[probe_ts]:.4f}")
            print(f"        - winner_stability_index_D: {signals_data['winner_stability_index_D'].loc[probe_ts]:.4f}")
            print(f"        - SLOPE_{bullish_magnitude_params.get('price_downtrend_slope_window', 5)}_close_D: {signals_data[f'SLOPE_{bullish_magnitude_params.get('price_downtrend_slope_window', 5)}_close_D'].loc[probe_ts]:.4f}")
            print(f"        - SLOPE_{bullish_magnitude_params.get('conviction_uptrend_slope_window', 5)}_main_force_conviction_index_D: {signals_data[f'SLOPE_{bullish_magnitude_params.get('conviction_uptrend_slope_window', 5)}_main_force_conviction_index_D'].loc[probe_ts]:.4f}")
            print(f"        - SLOPE_{bearish_magnitude_params.get('price_slope_window', 5)}_close_D: {signals_data[f'SLOPE_{bearish_magnitude_params.get('price_slope_window', 5)}_close_D'].loc[probe_ts]:.4f}")
            print(f"        - SLOPE_{bearish_magnitude_params.get('conviction_downtrend_slope_window', 5)}_main_force_conviction_index_D: {signals_data[f'SLOPE_{bearish_magnitude_params.get('conviction_downtrend_slope_window', 5)}_main_force_conviction_index_D'].loc[probe_ts]:.4f}")
            ...
            print(f"      [探针 - {method_name}] 最终 '牛市背离品质'分数 @ {probe_ts.strftime('%Y-%m-%d')}: {bullish_divergence_quality_final_score.loc[probe_ts]:.4f}")
            print(f"      [探针 - {method_name}] 最终 '熊市背离品质'分数 @ {probe_ts.strftime('%Y-%m-%d')}: {bearish_divergence_quality_final_score.loc[probe_ts]:.4f}")
    ...
    def _calculate_single_divergence_type(self, df: pd.DataFrame, is_bullish: bool, params: Dict, tf_weights: Dict, # tf_weights 已经作为参数传入
        ...
        if is_debug_enabled and probe_ts and not df.empty and probe_ts == df.index[-1]:
            ...
            print(f"        - ADX_14_D: {adx_val.loc[probe_ts]:.4f}")
            ...
            print(f"        - BBW_21_2.0_D: {signals_data['BBW_21_2.0_D'].loc[probe_ts]:.4f}")
            print(f"        - open_D: {open_price.loc[probe_ts]:.4f}")
            print(f"        - high_D: {high_price.loc[probe_ts]:.4f}")
            print(f"        - low_D: {low_price.loc[probe_ts]:.4f}")
            print(f"        - close_D: {close_price.loc[probe_ts]:.4f}")
