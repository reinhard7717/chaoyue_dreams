...
class CalculateCostAdvantageTrendRelationship:
    ...
    def _calculate_chip_barrier_solidity(self, df: pd.DataFrame, idx: pd.Index, is_debug: bool, probe_idx: int, temp_vals: Dict) -> np.ndarray:
        ...
        kinematic_mod = np.tanh(self._scale_by_volatility(pd.Series(slope_conc), 21) * 0.5 + self._scale_by_volatility(pd.Series(accel_conc), 21) * 0.3).astype(np.float32)
    ...
    def _calculate_predator_attack_vector(self, df: pd.DataFrame, idx: pd.Index, is_debug: bool, probe_idx: int, temp_vals: Dict) -> np.ndarray:
        ...
        hab_e_field = self._calc_hab_impact(pd.Series(e_field_raw), 21)
        ...
        v_norm = self._scale_by_volatility(pd.Series(v_price), 21)
        ...
        kinematic_mod = np.tanh(self._scale_by_volatility(pd.Series(slope_sm), 21) * 0.5 + self._scale_by_volatility(pd.Series(accel_sm), 21) * 0.3).astype(np.float32)
    ...
    def _calculate_kinematic_efficiency(self, df: pd.DataFrame, idx: pd.Index, is_debug: bool, probe_idx: int, temp_vals: Dict) -> np.ndarray:
        ...
        kinematic_mod = np.tanh(self._scale_by_volatility(pd.Series(slope_v), 21) * 0.5 + self._scale_by_volatility(pd.Series(accel_v), 21) * 0.3).astype(np.float32)
        mod_factor = np.clip(1.0 + primary_sign * kinematic_mod * 0.5, 0.0, 2.0)
    ...
    def _calculate_structure_negentropy(self, df: pd.DataFrame, idx: pd.Index, is_debug: bool, probe_idx: int, temp_vals: Dict) -> np.ndarray:
        ...
        hab_entropy = pd.Series(total_micro_entropy).rolling(window=34, min_periods=1).mean().to_numpy(dtype=np.float32)
        ...
        kinetics_mod = -np.tanh(self._scale_by_volatility(pd.Series(slope_ent), 21))
    ...
    def _calculate_pentagonal_resonance(self, D1: np.ndarray, D2: np.ndarray, D3: np.ndarray, D4: np.ndarray, D5: np.ndarray, df: pd.DataFrame, idx: pd.Index, is_debug: bool, probe_idx: int, temp_vals: Dict) -> np.ndarray:
        ...
        res_diff = pd.Series(base_resonance).diff(8).fillna(0.0).to_numpy(dtype=np.float32)
        price_diff = pd.Series(close).diff(8).fillna(0.0).to_numpy(dtype=np.float32)
        roll_cov = pd.Series(res_diff).rolling(13, min_periods=1).cov(pd.Series(price_diff)).fillna(0.0).to_numpy(dtype=np.float32)
        roll_var_res = pd.Series(res_diff).rolling(13, min_periods=1).var().fillna(0.0).to_numpy(dtype=np.float32)
        roll_var_price = pd.Series(price_diff).rolling(13, min_periods=1).var().fillna(0.0).to_numpy(dtype=np.float32)
