...
class CalculateCostAdvantageTrendRelationship:
    """
    【V15.0.0 · 绝对物理定律纠偏防反噬版】
    PROCESS_META_COST_ADVANTAGE_TREND
    - 用途: 基于高维物理场模型（谐振子势垒/洛伦兹偏转/卡诺热机/杨氏模量/结构熵）诊断趋势爆发力。
    - 本次修改要点:
      1. [零基陷阱切除] 建立双轨归一化防线。矢量使用 `_scale_by_volatility` 绝对保留 0 点极性，标量使用 `_center_and_scale`。
      2. [极性主导保护] 废除辅量与主量直接相加的逻辑。辅量仅能作为乘数放大或收缩主量，绝对禁止篡改主量极性。
      3. [物理常数兜底] 强制卡诺热机微观熵下限 0.1，阻断 Carnot_Eta=1.0 的绝对零度死锁。
      4. [量纲塌陷复原] 修复 D4 杨氏模量 Strain 因倍数错误引发的饱和死锁。
    """
    ...
    def _center_and_scale(self, series: pd.Series, window: int = 21) -> pd.Series:
        """【标量归一化】减去中位数。适用于无绝对物理方向的强度数据(如集中度)的相对波动计算"""
    ...
    def calculate(self, df: pd.DataFrame, config: Dict) -> pd.Series:
        ...
        print(f" ====== Start {method_name} 启动双轨极性物理张量共振计算，数据形状: {df.shape} ======")
        ...
        print(f" ====== End {method_name} 双轨极性物理张量共振计算完成 ======")
    ...
    def _calculate_predator_attack_vector(self, df: pd.DataFrame, idx: pd.Index, is_debug: bool, probe_ts: pd.Timestamp, temp_vals: Dict) -> pd.Series:
        ...
        # 极性主导保护：辅助因子只能放大或缩小原符号力量，无法篡改物理极性
    ...
    def _calculate_cost_migration_elasticity(self, df: pd.DataFrame, idx: pd.Index, is_debug: bool, probe_ts: pd.Timestamp, temp_vals: Dict) -> pd.Series:
        ...
        # 移除 100 乘子量纲灾难，重回真实的应变比例
        strain = (slope_c50 / (cost_50 + 1e-8)) * 10.0
    ...
    def _calculate_pentagonal_resonance(self, D1: pd.Series, D2: pd.Series, D3: pd.Series, D4: pd.Series, D5: pd.Series, df: pd.DataFrame, idx: pd.Index, is_debug: bool, probe_ts: pd.Timestamp, temp_vals: Dict) -> pd.Series:
        ...
        # 引入反身性横盘死区门限：如果价格几乎没动，不执行方向共振反馈
        price_dir = np.where(np.abs(price_slope / close) > 0.005, np.sign(price_slope), 0.0)
        alignment = np.where(base_resonance * price_dir > 0, 1.0, np.where(base_resonance * price_dir < 0, -1.0, 0.0))
        
        reflexivity_factor = pd.Series(np.where(alignment > 0, 1.0 + np.abs(raw_corr) * 0.5, 
                                       np.where(alignment < 0, 1.0 / (1.0 + np.abs(raw_corr) * 0.5), 1.0)), index=idx)
