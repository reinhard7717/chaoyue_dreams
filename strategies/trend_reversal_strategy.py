...
class TrendReversalStrategy(BaseStrategy):
    ...
    - 以指定的时间框架（默认为 '15'）为主要权重。
    - 结合量能确认或背离。
    """
    strategy_name = "TrendReversalStrategy"
    focus_timeframe = '15' # 主要关注的时间框架
    ...
    def get_required_columns(self) -> List[str]:
        ...
        all_score_indicators = bs_params.get('score_indicators', reversal_indicators) # 所有用于基础分的指标

        # 基础分计算所需指标列 (所有时间框架)
        for tf in timeframes:
             required.add(f'close_{tf}')
             if 'rsi' in all_score_indicators: required.add(f'RSI_{bs_params["rsi_period"]}_{tf}')
             if 'kdj' in all_score_indicators: required.update([f'K_{bs_params["kdj_period_k"]}_{bs_params["kdj_period_d"]}_{tf}', f'D_{bs_params["kdj_period_k"]}_{bs_params["kdj_period_d"]}_{tf}', f'J_{bs_params["kdj_period_k"]}_{bs_params["kdj_period_d"]}_{tf}'])
             if 'boll' in all_score_indicators: required.update([f'BB_UPPER_{tf}', f'BB_MIDDLE_{tf}', f'BB_LOWER_{tf}'])
        ...
        # 指标 OB/OS 反转信号所需 (在焦点时间框架)
        analysis_tf = self.focus_timeframe # 反转信号分析使用焦点时间框架
        required.add(f'close_{analysis_tf}')
        required.add(f'RSI_{bs_params["rsi_period"]}_{analysis_tf}')
        required.add(f'STOCH_K_{ia_params["stoch_k"]}_{analysis_tf}')
        required.add(f'CCI_{bs_params["cci_period"]}_{analysis_tf}')
        required.add(f'BB_UPPER_{analysis_tf}')
        required.add(f'BB_LOWER_{analysis_tf}')
        required.add(f'volume_{analysis_tf}')
        required.add(f'VOL_MA_{ia_params["volume_ma_period"]}_{analysis_tf}')
    ...
    def _calculate_reversal_focused_score(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 定义权重，焦点时间框架权重更高
        base_weight = (1.0 - tr_params.get('focus_weight', 0.6)) / (len(timeframes) - 1) if len(timeframes) > 1 else 0 # 焦点权重默认 0.6
        weights = {tf: base_weight for tf in timeframes if tf != self.focus_timeframe}
        weights[self.focus_timeframe] = tr_params.get('focus_weight', 0.6)
        ...
        for tf in timeframes:
            ...
            # (省略重复代码 - 与 TrendFollowing 类似，只是基于 all_score_indicators)
            if 'macd' in all_score_indicators:
    ...
    def _perform_signal_analysis(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        signal_weights = tr_params.get('signal_weights', {
            # 反转信号
            'regular_div': 0.25, # 常规背离
            'hidden_div': 0.0,   # 隐藏背离是持续信号，反转策略权重为0
            'obos_reversal': 0.15, # RSI/KDJ/CCI 等 OB/OS 反转
            'bb_reversal': 0.15, # 布林带反转
            'kline_reversal_strong': 0.2, # 强反转形态 (吞没, 星线, 锤子/上吊, 向上跳空两乌鸦, 反击线)
            'kline_reversal_weak': 0.1, # 弱反转形态 (刺透/乌云, 孕线, 镊子)
            # 持续信号 (在反转策略中权重低或为负)
            'kline_continuation': -0.05, # 持续形态可能阻碍反转 (三法, Tasuki, 分离线)
            'kline_marubozu': -0.1, # 光头光脚强烈指示当前趋势
            'kline_doji': 0.0,     # 十字星中性
            # 其他
            'volume_spike': 0.1,  # 放量确认反转信号
            'max_abs_signal': 1.0 # 限制确认信号绝对值最大为1
        })
        analysis_tf = self.focus_timeframe # 主要分析焦点时间框架的信号
        # --- 1. 背离检测 ---
        if dd_params.get('enabled', False):
            div_tf = dd_params.get('tf', analysis_tf) # 使用参数指定的或焦点TF
            ...
            if price_col not in data.columns or data[price_col].isnull().all():
                ...
                div_lookback = dd_params.get('lookback', 14)
                fp_params = strategy_utils.get_find_peaks_params(div_tf, div_lookback)

                for indicator_key, enabled in dd_params.get('indicators', {}).items():
                    if not enabled: continue
                    div_signal_col = f'{indicator_key}_divergence'
                    indicator_col = None
                    # 根据 indicator_key 获取对应的列名
                    if indicator_key == 'macd_hist': indicator_col = f'MACDh_{bs_params["macd_fast"]}_{bs_params["macd_slow"]}_{bs_params["macd_signal"]}_{div_tf}'
                    elif indicator_key == 'rsi': indicator_col = f'RSI_{bs_params["rsi_period"]}_{div_tf}'
                    elif indicator_key == 'mfi': indicator_col = f'MFI_{bs_params["mfi_period"]}_{div_tf}'
                    elif indicator_key == 'obv': indicator_col = f'OBV_{div_tf}'
                    # ... 其他可能的背离指标 ...

                    if indicator_col and indicator_col in data and data[indicator_col].notna().any():
                        analysis_df[div_signal_col] = strategy_utils.detect_divergence(
                            price_series, data[indicator_col],
                            lookback=div_lookback, find_peaks_params=fp_params,
                        ...
                        logger.debug(f"缺少指标列 {indicator_col} 或全为 NaN，无法计算 {indicator_key} 背离。")
        ...
        # --- 2. K 线形态检测 ---
        if kpd_params.get('enabled', False):
            kline_tf = kpd_params.get('tf', analysis_tf) # 使用参数指定的或焦点TF
        ...
        if price_col not in data or data[price_col].isnull().all():
            ...
            if bb_upper_col in data and bb_lower_col in data and data[bb_upper_col].notna().any() and data[bb_lower_col].notna().any():
                 ...
                 sell_cond = (close_series.shift(1) > upper.shift(1)) & (close_series <= upper) # 穿上轨回落
                 buy_cond = (close_series.shift(1) < lower.shift(1)) & (close_series >= lower) # 穿下轨回升
            ...
            if volume_col in data and vol_ma_col in data and data[volume_col].notna().any() and data[vol_ma_col].notna().any():
                 ...
                 vol_spike_factor = ia_params.get('volume_spike_factor', 2.0) # 默认2倍均量为放量
        ...
        if dd_params.get('enabled', False):
            for indicator_key in dd_params.get('indicators', {}).keys():
                if dd_params['indicators'].get(indicator_key):
                     ...
                     analysis_df['reversal_confirmation_signal'] -= (div_series == -1) * signal_weights.get('regular_div', 0)
                     # 隐藏: 反转策略中权重为0
                     # analysis_df['reversal_confirmation_signal'] += (div_series == 2) * signal_weights.get('hidden_div', 0)
                     # analysis_df['reversal_confirmation_signal'] -= (div_series == -2) * signal_weights.get('hidden_div', 0)
        # 指标 OB/OS 反转贡献 (使用新权重)
        ...
        # BB 反转贡献 (使用新权重)
        analysis_df['reversal_confirmation_signal'] += get_signal_value('bb_reversal') * signal_weights.get('bb_reversal', 0)
        # --- K 线形态贡献 (根据新形态和权重更新) ---
        ...
        # 强反转形态 (+/- w_rev_strong)
        analysis_df['reversal_confirmation_signal'] += (kline_series.isin([1, 3, 15])) * w_rev_strong      # 看涨: 吞没, 早晨星, 看涨反击
        analysis_df['reversal_confirmation_signal'] -= (kline_series.isin([-1, -3, -12, -15])) * w_rev_strong # 看跌: 吞没, 黄昏星, 跳空两乌鸦, 看跌反击
        analysis_df['reversal_confirmation_signal'] += (kline_series == 2) * w_rev_strong # 锤子
        analysis_df['reversal_confirmation_signal'] -= (kline_series == -2) * w_rev_strong # 上吊
        # 弱反转形态 (+/- w_rev_weak)
        analysis_df['reversal_confirmation_signal'] += (kline_series.isin([4, 7, 8, 9])) * w_rev_weak # 看涨: 刺透, 孕线, 十字孕线, 镊子底
        analysis_df['reversal_confirmation_signal'] -= (kline_series.isin([-4, -7, -8, -9])) * w_rev_weak # 看跌: 乌云, 孕线, 十字孕线, 镊子顶
        # 持续形态 (+/- w_cont) - 在反转策略中可能是负权重
        analysis_df['reversal_confirmation_signal'] += (kline_series.isin([6, 11, 13, 14])) * w_cont # 看涨持续: 红三兵, 升三法, Tasuki, 分离线
        analysis_df['reversal_confirmation_signal'] -= (kline_series.isin([-6, -11, -13, -14])) * w_cont # 看跌持续: 三乌鸦, 降三法, Tasuki, 分离线
        # 光头光脚 (+/- w_maru) - 在反转策略中可能是负权重
        analysis_df['reversal_confirmation_signal'] += (kline_series == 10) * w_maru # 光头阳
        analysis_df['reversal_confirmation_signal'] -= (kline_series == -10) * w_maru # 光头阴
        # 十字星 (Doji, 5) - 权重为0或忽略
        # 放量确认反转 (使用新权重)
        vol_spike = get_signal_value('volume_spike')
        current_signal_direction = np.sign(analysis_df['reversal_confirmation_signal'])
        # 放量且有反转信号时，加强该信号 (只加强正向反转确认)
        ...
        # 触发强确认信号
        threshold = tr_params.get('confirmation_threshold', 0.6) # 可以调整阈值
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.Series:
        ...
        # --- 步骤 3: 执行信号分析 (背离, K线, 指标反转) ---
        # 需要合并原始数据和基础分结果，因为信号分析可能用到原始指标值
        ...
        # 加权求和
        final_signal = 50.0 + (score_contribution * final_weights.get('base_score', 0.5) +
                           reversal_contribution * final_weights.get('reversal_confirm', 0.5))
    ...
    def analyze_signals(self) -> Optional[pd.DataFrame]:
        ...
        if 'final_signal' in data:
             ...
             if not final_signal.empty:
                  ...
                  analysis_results['final_signal_strong_buy_ratio'] = (final_signal >= 75).mean() # 更强反转信号
                  analysis_results['final_signal_strong_sell_ratio'] = (final_signal <= 25).mean() # 更强反转信号
        ...
        if latest_data is not None:
            ...
            # Summarize K-line stats for interpretation
            ...
            chinese_interpretation = (
                f"【趋势反转策略分析 - {pd.Timestamp.now().strftime('%Y-%m-%d %H:%M:%S')}】\n"
                f"核心观点:\n"
                ...
                # Add more stats as needed, e.g., divergence counts
            )
