...
class IntradayEngineOrchestrator:
    ...
    async def initialize_pools(self):
        """
        【盘前准备】构建监控池并存入Redis。
        这个任务应该在盘前由一个独立的Celery任务触发。
        """
        logger.info("盘中引擎开始盘前准备，正在构建监控池并存入Redis...")
        
        # 1. 构建“待买入池” (Watchlist)
        yesterday = date.today() # 实际应使用交易日历
        daily_buy_signals = await self.strategies_dao.get_daily_buy_signals(trade_date=yesterday)
        watchlist = {signal.stock.stock_code for signal in daily_buy_signals}
        
        # 2. 构建“持仓监控池” (Position List)
        ...
        for fav_stock in favorite_stocks:
            ...
            if stock_code not in position_list:
                position_list[stock_code] = {
                    ...
                    "user_id": fav_stock.user.id # 关联用户ID
                }
        ...
        async with self.cache_manager.redis_client.pipeline() as pipe:
            ...
            if position_list:
                # Redis Hash的field和value都必须是str, bytes或int/float
                # 我们将value序列化为JSON字符串
