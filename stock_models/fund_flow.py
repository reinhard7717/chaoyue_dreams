...
class FundFlowDaily(BaseModel):
    ...
    def __str__(self):
        return f"{self.stock.name if self.stock else ''}日级资金流向({self.trade_date})"

    def __code__(self):
        return self.stock.stock_code if self.stock else ''
    
class FundFlowDailyTHS(BaseModel):
    """
    ...
    def __str__(self):
        return f"{self.stock.name if self.stock else ''}日级资金流向 - 同花顺({self.trade_date})"

    def __code__(self):
        return self.stock.stock_code if self.stock else ''

class FundFlowDailyDC(BaseModel):
    """
    ...
    def __str__(self):
        return f"{self.stock.name if self.stock else ''}日级资金流向 - 东方财富({self.trade_date})"

    def __code__(self):
        return self.stock.stock_code if self.stock else ''

class FundFlowCntTHS(BaseModel):
    """
    ...
    index_close = models.DecimalField(max_digits=12, decimal_places=4, verbose_name=_("板块指数"), null=True, blank=True)
    ...
    def __str__(self):
        return f"{self.stock.name if self.stock else ''}板块资金流向统计 - 同花顺({self.trade_date})"

    def __code__(self):
        return self.stock.stock_code if self.stock else ''

class FundFlowCntDC(BaseModel):
    """
...
class FundFlowIndustryTHS(BaseModel):
    """
    ...
    trade_date = models.DateField(verbose_name=_("交易日期"), null=True, blank=True)
    industry_name = models.CharField(max_length=20, verbose_name=_("行业名称"), null=True, blank=True)
    lead_stock = models.CharField(max_length=20, verbose_name=_("领涨股"), null=True, blank=True)
    ...
    def __str__(self):
        return f"{self.stock.name if self.stock else ''}行业资金流向统计 - 同花顺({self.trade_date})"

    def __code__(self):
        return self.stock.stock_code if self.stock else ''

class FundFlowMarketDc(BaseModel):
    """
    ...
    def __str__(self):
        return f"{self.stock.name if self.stock else ''}市场资金流向统计 - 东方财富({self.trade_date})"
