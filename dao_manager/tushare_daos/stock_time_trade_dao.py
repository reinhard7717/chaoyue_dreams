...
class StockTimeTradeDAO(BaseDAO):
    ...
    async def save_minute_time_trade_history_by_stock_codes(self, stock_codes: List[str]) -> None:
        ...
        for time_level in time_levels:
            ...
            while True:
                ...
                df = df.dropna(subset=['ts_code', 'trade_time'], how='any')  # 去掉无效行
    ...
    async def save_minute_time_trade_history_by_stock_code_and_time_level(self, stock_code: str, time_level: str) -> None:
        """
        保存股票的历史分钟级交易数据
        接口：stk_mins
        描述：获取A股分钟数据，支持1min/5min/15min/30min/60min行情，提供Python SDK和 http Restful API两种方式
