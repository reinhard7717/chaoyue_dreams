        ...
        try:
            ...
            if training_successful and \
               ...
               strategy.selected_feature_names_for_transformer:
                ...
                logger.error(f"[{item_name}] Transformer 模型训练逻辑执行完毕，但未能成功标记为训练或必要组件缺失。")
                print(f"ERROR: [{item_name}] 训练方法返回失败或组件未正确设置。Training successful flag: {training_successful}")
                failed_training_count += 1
        except Exception as e:
            logger.error(f"为股票 {item_name} 执行 Transformer 模型训练时发生意外错误: {e}", exc_info=True)
            print(f"ERROR: [{item_name}] 训练过程中发生异常: {e}")
            failed_training_count += 1
    # 修改结束：排序和遍历逻辑已更新
...
if __name__ == '__main__':
    ...
    print(f"INFO: 命令行参数已解析。Params File: '{args.params_file}', Strategy Data Dir: '{args.strategy_data_dir}', Order: '{args.order}'") # 修改行：打印新的参数值
